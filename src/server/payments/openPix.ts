...
export class OpenPixProvider implements ServerPaymentProvider {
  ...
  constructor(private readonly appId: string, mode: OpenPixMode = 'sandbox') {
    this.baseUrl = mode === 'production' ? 'https://api.openpix.com.br/api/v1' : 'https://api.woovi-sandbox.com/api/v1'
  }
  ...
  async createPayment(input: ServerCreatePaymentInput): Promise<ServerCreatedPayment> {
    ...
    return {
      processor: this.id,
      externalPaymentId: String(charge?.correlationID || charge?.identifier || correlationID),
  ...
  async getPaymentStatus(externalPaymentId: string): Promise<ServerPaymentStatusResult> {
    ...
    const charge = data?.charge || data
    return { processor: this.id, externalPaymentId, status: mapOpenPixStatus(charge?.status), endToEndId: charge?.transaction?.endToEndId || charge?.endToEndId || null, raw: data }
  }
  ...
  async refundPayment(input: ServerRefundInput): Promise<ServerRefundResult> {
    const response = await fetch(`${this.baseUrl}/charge/${encodeURIComponent(input.externalPaymentId)}/refund`, {
      method: 'POST',
      headers: this.headers(),
      body: JSON.stringify({ correlationID: input.idempotencyKey, ...(input.amount ? { value: cents(input.amount) } : {}), ...(input.reason ? { comment: input.reason.slice(0, 140) } : {}) }),
    })
    const data = await response.json().catch(() => null)
    ...
    return { processor: this.id, externalPaymentId: input.externalPaymentId, refundId: refund?.correlationID || refund?.endToEndId || null, status: ['REFUNDED', 'DEVOLVIDO', 'COMPLETED'].includes(status) ? 'refunded' : 'pending', raw: data }
  }
}

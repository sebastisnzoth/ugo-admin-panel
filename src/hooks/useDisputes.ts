...
export function useAdminDisputes(){
 ...
 const refetch=useCallback(async()=>{setLoading(true);setError(null);const{data,error:e}=await(adminSupabase as any).from('disputas').select('id,numero,servicio_id,cliente_id,proveedor_id,abierta_por,estado,motivo,monto_disputado,resolucion,resolucion_favor,ajuste_financiero_pendiente,created_at,resuelta_at,clientes:usuarios!disputas_cliente_id_fkey(nombre),proveedores:usuarios!disputas_proveedor_id_fkey(nombre)').in('estado',['abierta','en_revision']).order('created_at',{ascending:true});if(e)setError(e.message);else setDisputes((data||[])as DisputeRow[]);setLoading(false)},[])

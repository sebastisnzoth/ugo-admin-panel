...
const PROVIDER_ATTENTION_TYPES=new Set(['nueva_oferta','trabajo_asignado'])
...
export function NotificationCenter({role,onOpenNotice,attentionEnabled=true}:Props){
 ...
 useEffect(()=>{if(!attentionEnabled)return;const arm=()=>{const ctx=role==='provider'?providerAudio():clientAudio();if(ctx?.state==='suspended')void ctx.resume().catch(()=>{})};window.addEventListener('pointerdown',arm,true);window.addEventListener('keydown',arm,true);return()=>{window.removeEventListener('pointerdown',arm,true);window.removeEventListener('keydown',arm,true)}},[attentionEnabled,role])
 ...
 const pushLabel=pushState==='on'?'✓ Avisos activados':pushState==='loading'?'Activando…':pushState==='blocked'?'Avisos bloqueados':pushState==='unsupported'?'Push no disponible':'Activar avisos',providerCall=Boolean(liveNotice&&role==='provider'&&PROVIDER_ATTENTION_TYPES.has(liveNotice.tipo)),clientCall=Boolean(liveNotice&&role==='client'&&CLIENT_ATTENTION_TYPES.has(liveNotice.tipo))
 return <div className={`ugo-notification-center role-${role}`}>
  ...
  {liveNotice&&<button type="button" className={`ugo-notification-live is-${role} ${providerCall?'is-provider-call':''}`} onClick={()=>void openNotice(liveNotice)} aria-live="assertive"><span className="ugo-notification-icon">{icon(liveNotice.tipo)}</span><span><small>{providerCall?'UGO · NUEVO PEDIDO':clientCall?(liveNotice.tipo==='chat_mensaje'?'UGO · MENSAJE NUEVO':'UGO · ESTADO DEL PEDIDO'):'UGO · ACTUALIZACIÓN EN VIVO'}</small><strong>{liveNotice.titulo}</strong>{liveNotice.cuerpo&&<em>{liveNotice.cuerpo}</em>}<b>{providerCall?(liveNotice.tipo==='nueva_oferta'?'Ver y decidir →':'Abrir trabajo →'):'Ver pedido →'}</b></span></button>}

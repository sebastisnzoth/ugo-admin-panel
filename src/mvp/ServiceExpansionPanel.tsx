...
export function ServiceExpansionPanel({role,serviceId,compact=false}:{role:UgoRole;serviceId?:string|null;compact?:boolean}){
 ...
 async function resolve(id:string,approve:boolean){const item=items.find(x=>x.id===id);if(approve&&item&&needsElectronicAdjustment(item)){setMessage('Este servicio ya tiene un pago electrónico activo. UGO debe cobrar primero el monto adicional antes de poder aprobarlo.');return}setBusy(true);setMessage('');const{error}=await sb.rpc('resolver_ampliacion_servicio',{p_ampliacion_id:id,p_aprobar:approve});setBusy(false);if(error){setMessage(error.message);return}setMessage(approve?'Trabajo adicional aprobado.':'Trabajo adicional rechazado.');await load()}
 const body=<div className="ugo-expansion-body">
  {items.length>0&&<div className="ugo-expansion-list">{items.map(x=>{const adjustmentBlocked=needsElectronicAdjustment(x);return <article key={x.id} className={`ugo-expansion-item state-${x.estado}`}><div className="ugo-expansion-head"><b>{x.propuesto_por_rol==='cliente'?'Pedido del cliente':'Propuesta del proveedor'}</b><span>{x.estado}</span></div><p>{x.descripcion}</p><div className="ugo-expansion-meta"><strong>+ {money(x.monto_extra,service.moneda||'BRL')}</strong><span>+ {x.minutos_extra} min</span></div>{adjustmentBlocked&&<small>⚠ El servicio ya tiene un pago electrónico activo. El monto adicional necesita un cobro de ajuste antes de poder aprobarse.</small>}{x.pago_estado==='pendiente_ajuste'&&<small>⚠ Estado histórico: existe un ajuste financiero pendiente. UGO no permite cerrar el servicio hasta reconciliarlo.</small>}{x.pago_estado==='incluido'&&x.estado==='aprobada'&&<small>✓ Importe incorporado al total del servicio.</small>}{role==='client'&&x.estado==='pendiente'&&<div className="ugo-expansion-actions"><button disabled={busy} onClick={()=>resolve(x.id,false)}>Rechazar</button><button className="primary" disabled={busy||adjustmentBlocked} onClick={()=>resolve(x.id,true)}>{adjustmentBlocked?'Falta cobrar ajuste':'Aprobar'}</button></div>}</article>})}</div>}

...
function sessionKey(role:string,token?:string){
  let uid='anon'
  try{const payload=JSON.parse(atob(String(token||'').split('.')[1]?.replace(/-/g,'+').replace(/_/g,'/')||''));uid=String(payload?.sub||'anon')}catch{}
  return `ugo:hugo:history:${role}:${uid}`
}
...
export function useHugoVoice({ role, accessToken, context }: VoiceOptions) {
  ...
  const connect=useCallback(async()=>{if(state!=='idle'&&state!=='error')return;if(!accessToken){setError('Iniciá sesión para hablar con Hugo.');setState('error');return}const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){setError('Este navegador no ofrece reconocimiento de voz. Probá Chrome, Edge o Safari actualizado.');setState('error');return}
    setState('connecting');setError('');setUserTranscript('');setAssistantTranscript('');runningRef.current=true;historyRef.current=readHistory(historyKeyRef.current);const recognition=new SR();recognition.lang='es-AR';recognition.continuous=false;recognition.interimResults=true;recognitionRef.current=recognition

...
export function ClientGlobalMenu(){
 ...
 return <>
  ...
  {open&&<div className="ugo-client-menu-backdrop ugo-client-global-backdrop" onClick={close} role="presentation"><aside className="ugo-client-menu ugo-client-global-drawer" onClick={e=>e.stopPropagation()} aria-label="Menú UGO Cliente">
   ...
   <button type="button" className="ugo-client-account-card" onClick={profile}><span className="ugo-client-account-avatar">C</span><div><b>Mi cuenta</b><small>Perfil y preferencias</small></div><em>›</em></button>
   <button type="button" className="ugo-client-menu-primary" onClick={search}><span>✎</span><div><b>Pedir con Hugo</b><small>Escribí o hablá sobre lo que necesitás</small></div><em>›</em></button>
   <div className="ugo-client-menu-section-label">Tu actividad</div>
   <nav className="ugo-client-menu-list ugo-client-global-list"><button type="button" onClick={home}><span>⌂</span><div><b>Inicio</b><small>Volver a Hugo</small></div><em>›</em></button><button type="button" onClick={history}><span>◷</span><div><b>Actividad</b><small>Pedidos activos, cancelados e historial</small></div><em>›</em></button><button type="button" onClick={location}><span>⌖</span><div><b>Mi ubicación</b><small>Actualizar dónde estás</small></div><em>›</em></button></nav>
   <div className="ugo-client-menu-section-label">Ayuda</div>
   <nav className="ugo-client-menu-list ugo-client-global-list ugo-client-global-list-secondary"><button type="button" onClick={dispute}><span>?</span><div><b>Ayuda y disputas</b><small>Problemas con un servicio</small></div><em>›</em></button><button type="button" onClick={hugo}><span>✦</span><div><b>Hablar con Hugo</b><small>Continuar la conversación por voz</small></div><em>›</em></button></nav>

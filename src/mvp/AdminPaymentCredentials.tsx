...
export function AdminPaymentCredentials(){
 ...
 const save=async(provider:ProviderId)=>{
  const d=drafts[provider],m=meta[provider],credentials:Record<string,string>={}
  ...
  const{error}=await rpcClient.rpc('admin_set_payment_credentials',{p_provider:provider,p_country:m.country,p_environment:d.environment,p_credentials:credentials,p_enabled:d.enabled})

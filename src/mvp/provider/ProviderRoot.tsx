...
function ProviderOperationalRoot(){
 ...
 useEffect(()=>flow.registerActions({openHome:()=>flow.navigate('home'),openDemand:()=>flow.navigate('demand'),openOpportunities:()=>flow.navigate('opportunities'),openOpportunity:id=>flow.navigate('opportunity-detail',id),acceptOpportunity:async id=>{const ok=await data.acceptOpportunity(id);if(ok)flow.navigate('active-job');return ok},rejectOpportunity:async id=>{const ok=await data.rejectOpportunity(id);if(ok)flow.navigate('opportunities');return ok},openActiveJob:()=>flow.navigate('active-job'),openAgenda:()=>flow.navigate('agenda'),openEarnings:()=>flow.navigate('earnings'),openProfile:()=>flow.navigate('profile'),openHistory:()=>flow.navigate('history'),openDispute:()=>flow.navigate('dispute')}),[data.acceptOpportunity,data.rejectOpportunity,flow.navigate,flow.registerActions])

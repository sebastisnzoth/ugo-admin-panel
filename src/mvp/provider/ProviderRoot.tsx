...
function ProviderOperationalRoot(){
 ...
 return <div className="ugo-provider-root">{data.notice&&<div className={`provider-notice ${data.notice.type}`}>{data.notice.text}</div>}{screen==='home'&&<ProviderHome/>}{screen==='demand'&&<ProviderDemand/>}{screen==='opportunities'&&<ProviderOpportunities/>}{screen==='opportunity-detail'&&<ProviderOpportunityDetail id={flow.opportunityId}/>} {screen==='active-job'&&<ProviderActiveJob/>}{screen==='earnings'&&<section className="provider-screen"><span className="provider-kicker">GANANCIAS</span><h1>Tu dinero</h1><div className="provider-grid"><article className="provider-card"><small>PROTEGIDO</small><strong>{money(data.retained)}</strong></article><article className="provider-card"><small>LIBERADO</small><strong>{money(data.released)}</strong></article></div></section>}{screen==='profile'&&<section className="provider-screen"><span className="provider-kicker">PERFIL PROFESIONAL</span><h1>{data.name}</h1><article className="provider-card"><strong>⭐ {data.karma.toFixed(1)}</strong><p>{data.provider.ciudad_base||'Ciudad no definida'}</p><p>Radio de trabajo: {data.provider.zona_radio_km||15} km</p></article></section>}<ServiceHistoryPanel role="provider" openRequest={screen==='history'}/><DisputeDock role="provider" openRequest={screen==='dispute'}/><AppLocationButton role="provider"/><nav className="provider-bottom-nav" aria-label="Navegación proveedor"><button className={screen==='home'?'active':''} onClick={flow.actions.openHome}>Inicio</button><button className={screen==='demand'||screen==='opportunities'||screen==='opportunity-detail'?'active':''} onClick={flow.actions.openDemand}>Demanda{data.opportunities.length>0&&<b className="provider-nav-badge">{data.opportunities.length}</b>}</button><button className={screen==='active-job'||screen==='history'?'active':''} onClick={data.service?flow.actions.openActiveJob:flow.actions.openHistory}>Trabajos</button><button className={screen==='profile'?'active':''} onClick={flow.actions.openProfile}>Perfil</button></nav></div>
}

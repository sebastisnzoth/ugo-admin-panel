...
export function ProviderAgenda(){
 ...
 useEffect(()=>{let alive=true;let channel:ReturnType<typeof db.channel>|null=null;db.auth.getSession().then(({data})=>{const id=data.session?.user.id||null;if(!alive)return;setUserId(id);if(!id){setLoading(false);return}void load(id);channel=db.channel(`provider-agenda-${id}`).on('postgres_changes',{event:'*',schema:'public',table:'servicios',filter:`proveedor_id=eq.${id}`},()=>void load(id)).subscribe(status=>{if(status==='SUBSCRIBED')void load(id)})}).catch(sessionError=>{if(alive){setError(sessionError instanceof Error?sessionError.message:'No pudimos iniciar tu agenda.');setLoading(false)}});return()=>{alive=false;if(channel)void db.removeChannel(channel)}},[db,load])
 ...
 return <section className="provider-screen" aria-labelledby="provider-agenda-title">
  <header className="provider-section-head"><div><span className="provider-kicker">AGENDA UGO</span><h1 id="provider-agenda-title">Próximos trabajos</h1><p>Horarios confirmados vinculados a tu cuenta. Cada tarjeta conserva su propio serviceId.</p></div><button type="button" className="provider-link" onClick={flow.actions.openHistory}>Ver historial</button></header>

...
export function ProviderActiveJob(){
 ...
 return <section className="provider-screen provider-active-job" aria-labelledby="provider-job-title">
  ...
  <Card className="provider-card provider-job-control-card provider-active-control" aria-label="Cambiar estado del pedido">
   ...
   {s.estado==='disputado'&&<div className="provider-simple-done" role="status"><strong>⚖ Disputa abierta</strong><span>UGO pausó el cierre normal. Revisá el expediente y respondé desde el mismo caso.</span><Button variant="secondary" className="provider-secondary provider-wide" onClick={()=>flow.actions.openDispute(s.id)}>VER DISPUTA</Button></div>}

...
export function ProviderActiveJob(){
 ...
 const confirmCashReceived=async()=>{if(d.busy||!d.cashSelected||d.cashConfirmed)return;if(!window.confirm('¿Confirmás que el cliente te pagó en efectivo y que recibiste el dinero? Esta acción queda registrada.'))return;await d.confirmCash()}
 ...
 return <section className="provider-screen provider-active-job" aria-labelledby="provider-job-title">
  ...
  <section className="provider-card provider-job-control-card provider-active-control" aria-label="Cambiar estado del pedido">
   ...
   {s.estado==='en_progreso'&&evidence.final&&d.cashSelected&&!d.cashConfirmed&&<button type="button" className="provider-primary provider-main-action" disabled={d.busy} onClick={()=>void confirmCashReceived()}>{d.busy?'Registrando…':'EL CLIENTE PAGÓ'}</button>}
   ...
   {s.estado==='en_progreso'&&d.cashSelected&&evidence.final&&!d.cashConfirmed&&<p className="provider-action-note">Tocá “EL CLIENTE PAGÓ” sólo cuando realmente hayas recibido el efectivo. UGO lo registra en el pedido.</p>}

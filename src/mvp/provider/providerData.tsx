...
export function ProviderDataProvider({children}:{children:React.ReactNode}){
 ...
 const advance=(state:'en_camino'|'llegado'|'en_progreso'|'esperando_aprobacion')=>{if(!service)return Promise.resolve(false);if(service.estado==='asignado'&&!funded&&!cashSelected){setNotice({type:'info',text:'Todavía falta que el cliente confirme la forma de pago.'});return Promise.resolve(false)}if(state==='esperando_aprobacion'&&cashSelected&&!cashConfirmed){setNotice({type:'info',text:'Primero confirmá que recibiste el efectivo. Después UGO habilita la aprobación del cliente.'});return Promise.resolve(false)}return run(()=>advanceProviderService(supabase,service.id,state),'Estado del servicio actualizado.')}
 const confirmCash=async()=>{if(!service)return false;setBusy(true);setNotice(null);try{const{error}=await supabase.rpc('confirmar_pago_efectivo',{p_servicio_id:service.id});if(error)throw error;setNotice({type:'ok',text:'Efectivo recibido y registrado. El cliente ya puede revisar y aprobar el cierre.'});await reload();return true}catch(e){setNotice({type:'error',text:e instanceof Error?e.message:'No se pudo confirmar el efectivo.'});return false}finally{setBusy(false)}}
 const opportunities:ProviderOpportunity[]=offers.map(o=>({id:o.id,serviceId:o.servicio_id,category:o.servicio?.categoria?.nombre||'Servicio',title:o.servicio?.descripcion||'Nueva oportunidad',description:o.servicio?.descripcion||'',zone:o.servicio?.direccion_cliente||'Zona por confirmar',distanceKm:Number(o.distancia_km||0),estimatedValue:Number(o.tarifa_ofrecida||o.servicio?.tarifa||0),requestedAt:o.servicio?.created_at||new Date().toISOString(),urgency:o.servicio?.urgencia?'urgent':'normal',matchScore:o.ranking==null?undefined:Number(o.ranking)}))

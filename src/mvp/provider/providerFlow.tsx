...
const emptyActions:ProviderActionHandlers={openHome:noop,openDemand:noop,openOpportunities:noop,openOpportunity:noop,acceptOpportunity:unavailable,rejectOpportunity:unavailable,openActiveJob:noop,openAgenda:noop,openAgendaService:noop,openEarnings:noop,openProfile:noop,openHistory:noop,openDispute:noop}

type ProviderFlow={screen:ProviderScreen;opportunityId:string|null;agendaServiceId:string|null;historyServiceId:string|null;disputeServiceId:string|null;actions:ProviderActionHandlers;navigate:(screen:ProviderScreen,id?:string|null)=>void;registerActions:(actions:Partial<ProviderActionHandlers>)=>()=>void}
...
export function ProviderFlowProvider({children}:{children:React.ReactNode}){
 ...
 const[agendaServiceId,setAgendaServiceId]=useState<string|null>(null)
 ...
 const navigate=useCallback((next:ProviderScreen,id:string|null=null)=>{setScreen(next);setOpportunityId(next==='opportunity-detail'?id:null);setAgendaServiceId(next==='agenda'?id:null);setHistoryServiceId(next==='history'?id:null);setDisputeServiceId(next==='dispute'?id:null)},[])
 ...
 return <ProviderFlowContext.Provider value={{screen,opportunityId,agendaServiceId,historyServiceId,disputeServiceId,actions,navigate,registerActions}}>{children}</ProviderFlowContext.Provider>
}

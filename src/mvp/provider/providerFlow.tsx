...
type ProviderFlow={screen:ProviderScreen;opportunityId:string|null;agendaServiceId:string|null;historyServiceId:string|null;actions:ProviderActionHandlers;navigate:(screen:ProviderScreen,id?:string|null)=>void;registerActions:(actions:Partial<ProviderActionHandlers>)=>()=>void}
...
export function ProviderFlowProvider({children}:{children:React.ReactNode}){
 ...
 const navigate=useCallback((next:ProviderScreen,id:string|null=null)=>{setScreen(next);setOpportunityId(next==='opportunity-detail'?id:null);setAgendaServiceId(next==='agenda'?id:null);setHistoryServiceId(next==='history'?id:null)},[])
 ...
 return <ProviderFlowContext.Provider value={{screen,opportunityId,agendaServiceId,historyServiceId,actions,navigate,registerActions}}>{children}</ProviderFlowContext.Provider>
}

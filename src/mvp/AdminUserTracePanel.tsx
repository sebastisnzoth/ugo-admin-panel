...
type Document={id:string;tipo:string;storage_path:string;nombre_archivo:string|null;mime_type:string|null;estado:string;notas_revision:string|null;created_at:string;updated_at:string|null;revisado_at:string|null;url?:string|null}
...
export function AdminUserTracePanel({user,onClose}:{user:User;onClose:()=>void}){
 ...
 const load=useCallback(async()=>{
  ...
  try{
   ...
   const docQuery=user.tipo==='proveedor'?(supabase as any).from('documentos_proveedor').select('id,tipo,storage_path,nombre_archivo,mime_type,estado,notas_revision,created_at,updated_at,revisado_at').eq('usuario_id',user.id).order('created_at',{ascending:false}):Promise.resolve({data:[],error:null})
   ...
   const signedDocs=await Promise.all(((docRows||[])as Document[]).map(async row=>{const{data}=await supabase.storage.from('provider-kyc').createSignedUrl(row.storage_path,1800);return{...row,url:data?.signedUrl||null}}))
 ...
 return <div className="ugo-admin-user-trace-backdrop" role="presentation" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}>
  <section className="ugo-admin-user-trace" role="dialog" aria-modal="true" aria-label={`Historial de ${user.nombre}`}>
   ...
   {loading?<div className="ugo-admin-user-trace-loading">Cargando historial completo…</div>:<>
    ...
    {user.tipo==='proveedor'&&<section className="ugo-admin-user-trace-section"><header><strong>Documentación</strong><span>{documents.length}</span></header><div className="ugo-admin-user-docs">{documents.map(d=><article key={d.id}><div><strong>{d.tipo.replaceAll('_',' ')}</strong><span>{d.estado}</span></div><small>Subido {when(d.created_at)}{d.revisado_at?` · Revisado ${when(d.revisado_at)}`:''}</small>{d.notas_revision&&<p>{d.notas_revision}</p>}{d.url&&<a href={d.url} target="_blank" rel="noreferrer">Ver documento</a>}</article>)}{!documents.length&&<p className="empty">Sin documentos registrados.</p>}</div></section>}

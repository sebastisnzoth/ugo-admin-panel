...
export function AdminServicesPro(){
 const{services,loading,error,refetch}=useAdminActiveServices();const[q,setQ]=useState('');const[state,setState]=useState('todos')
 ...
 return <div className="ugo-services-pro">
  <div className="ugo-services-pro-toolbar"><div><small>CONTROL DE SERVICIOS</small><strong>{services.length} servicios activos cargados</strong></div><button onClick={()=>{void refetch()}} disabled={loading}>{loading?'Actualizando…':'↻ Actualizar'}</button></div>
  ...
  {error?<div className="ugo-services-pro-empty"><strong>No se pudieron cargar los servicios</strong><span>{error}</span><button onClick={()=>{void refetch()}}>Reintentar</button></div>:loading&&!services.length?<div className="ugo-services-pro-empty">Cargando operación…</div>:!rows.length?<div className="ugo-services-pro-empty"><strong>No hay servicios para este filtro</strong><span>Probá otro estado o limpiá la búsqueda.</span></div>:<div className="ugo-services-pro-list">{rows.map((s:any)=><article key={s.id}><div className="ugo-services-pro-main"><div className={`ugo-services-pro-status status-${s.estado}`}><i/>{label(s.estado)}</div><h3>Servicio #{s.numero||'—'} · {s.categoria?.emoji} {s.categoria?.nombre||'Servicio'}</h3><p>{s.descripcion||'Sin descripción adicional.'}</p>{s.direccion_cliente&&<small>{s.direccion_cliente}</small>}</div><div className="ugo-services-pro-people"><small>CLIENTE</small><strong>{fullName(s.cliente)}</strong><small>PROVEEDOR</small><strong>{s.proveedor?fullName(s.proveedor):'Sin asignar'}</strong></div><div className="ugo-services-pro-meta"><small>TARIFA</small><strong>{money(s.tarifa)}</strong><small>ACTUALIZADO</small><span>{when(s.updated_at||s.created_at)}</span></div></article>)}</div>}
 </div>
}

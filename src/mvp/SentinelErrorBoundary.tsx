...
type Props={children:React.ReactNode;role?:'client'|'provider'|'admin';serviceId?:string|null;checklistCode?:string|null;action?:string|null;title?:string;compact?:boolean;onClose?:()=>void}
...
export class SentinelErrorBoundary extends React.Component<Props,State>{
 ...
 componentDidCatch(error:Error,info:React.ErrorInfo){
  void reportSentinelIncident({eventType:'module_render_error',message:error.message||'Un módulo del servicio no pudo renderizarse',error,role:this.props.role||'unknown',severity:this.props.checklistCode==='CLIENT-ORDER-OPEN'?'P0':'P1',serviceId:this.props.serviceId,action:this.props.action,checklistCode:this.props.checklistCode,metadata:{componentStack:info.componentStack||''}})
 }
 render(){

...
export function ProviderEvidencePanel({service}:Props){
 ...
 return <div style={{position:'fixed',left:12,top:'max(78px, calc(env(safe-area-inset-top) + 68px))',zIndex:79}}>
  ...
  {open&&<div style={{position:'absolute',left:0,top:52,width:320,maxWidth:'calc(100vw - 24px)',maxHeight:'calc(100dvh - 150px)',overflowY:'auto',background:'#fff',borderRadius:20,padding:14,boxShadow:'0 14px 40px rgba(0,0,0,.2)',color:'#101828'}}>
   ...
   <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><button type="button" disabled={busy} onClick={()=>void takePhoto()} style={{minHeight:48,border:`1px dashed ${finalRequired?'#f79009':'#bbb'}`,borderRadius:12,fontWeight:700,background:finalRequired?'#fffaeb':'#fff'}}>📷 Sacar foto</button><label style={{display:'grid',placeItems:'center',minHeight:48,padding:12,border:`1px dashed ${finalRequired?'#f79009':'#bbb'}`,borderRadius:12,textAlign:'center',cursor:'pointer',fontWeight:700,background:finalRequired?'#fffaeb':'#fff'}}>{busy?'Subiendo…':'＋ Elegir foto'}<input type="file" accept="image/*" disabled={busy} onChange={e=>{void upload(e.target.files?.[0]||null);e.currentTarget.value=''}} style={{display:'none'}}/></label></div>

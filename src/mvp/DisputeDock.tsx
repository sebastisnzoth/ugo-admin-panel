...
export function DisputeDock({role,openRequest=false,serviceId=null}:{role:UgoRole;openRequest?:boolean;serviceId?:string|null}){
 const{service,dispute,messages,loading,error,ambiguous,open,reply}=useParticipantDispute(role,serviceId)
 ...
 const submit=async()=>{if(!service)return setNotice('Elegí el pedido exacto desde Actividad antes de abrir una disputa.');if(text.trim().length<8)return setNotice('Contá un poco más qué pasó.');setBusy(true);setNotice(null);try{if(dispute)await reply(text.trim());else await open(text.trim());setText('');setNotice(dispute?'Respuesta enviada al caso.':'Disputa abierta. UGO detuvo el flujo normal de este servicio para revisión.')}catch(e){setNotice(e instanceof Error?e.message:'No se pudo registrar la disputa.')}finally{setBusy(false)}}
 return <>

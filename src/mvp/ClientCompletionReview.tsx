...
export function ClientCompletionReview({onOpenDispute,serviceId=null}:{onOpenDispute?:()=>void;serviceId?:string|null}){
 ...
 const load=useCallback(async()=>{
  ...
  let query=supabase.from('servicios').select('id,numero,estado,proveedor_id').eq('cliente_id',uid)
 ...
 useEffect(()=>{if(!userId)return;const refresh=()=>void load().catch(()=>{});const serviceFilter=serviceId?`id=eq.${serviceId}`:`cliente_id=eq.${userId}`;const ch=supabase.channel(`client-completion-review-${serviceId||userId}`).on('postgres_changes',{event:'*',schema:'public',table:'servicios',filter:serviceFilter},refresh).on('postgres_changes',{event:'*',schema:'public',table:'evidencias_servicio',...(serviceId?{filter:`servicio_id=eq.${serviceId}`}:{})},refresh).on('postgres_changes',{event:'*',schema:'public',table:'pagos',...(serviceId?{filter:`servicio_id=eq.${serviceId}`}:{filter:`cliente_id=eq.${userId}`})},refresh).subscribe(status=>{if(status==='SUBSCRIBED')refresh()});const onOnline=()=>refresh();const onVisibility=()=>{if(document.visibilityState==='visible')refresh()};window.addEventListener('online',onOnline);document.addEventListener('visibilitychange',onVisibility);return()=>{window.removeEventListener('online',onOnline);document.removeEventListener('visibilitychange',onVisibility);supabase.removeChannel(ch)}},[load,serviceId,supabase,userId])
 ...
 const canApprove=!completed&&service.estado==='esperando_aprobacion'&&hasFinalEvidence&&Boolean(payment)&&(!isCash||cashConfirmed)
 ...
 return <section aria-live="polite" className="ugo-completion-review">
  <header><div><h2>{completed?'Trabajo confirmado':'¿Cómo quedó el trabajo?'}</h2><p>{completed?`Servicio #${service.numero} · El pedido quedó cerrado y las fotos siguen disponibles en su historial.`:`Servicio #${service.numero} · Revisá el registro final antes de cerrar el servicio.`}</p></div><span aria-hidden="true">✓</span></header>
  ...
  {!completed&&isCash&&!cashConfirmed&&<div className="ugo-completion-warning">El proveedor todavía debe confirmar que recibió el efectivo. UGO registra este pago, pero no tiene custodia electrónica sobre el dinero.</div>}
  {notice&&<div className="ugo-completion-notice">{notice}</div>}
  {!completed&&<><div className="ugo-completion-decision"><span>{blockedReason||(isCash?'El proveedor confirmó que recibió el efectivo. Confirmá el trabajo para cerrar el servicio.':'El trabajo está listo para confirmar. Al aprobar, UGO libera el pago electrónico protegido.')}</span></div><div className="ugo-completion-actions">{onOpenDispute&&<button type="button" onClick={onOpenDispute} disabled={busy}>Tengo un problema</button>}<button type="button" onClick={approve} disabled={busy||!canApprove}>{busy?'Procesando…':isCash?'CONFIRMAR TRABAJO':'CONFIRMAR Y LIBERAR PAGO'}</button></div></>}

...
export function AdminDisputesDecisionCenter(){
 ...
 return <div className="ugo-decision">
  ...
  {selected&&<div className="ugo-resolution"><div><small>RESOLUCIÓN ADMINISTRATIVA</small><h3>Caso #{selected.numero||String(selected.id).slice(0,8)}</h3><p>La resolución se registra en UGO y se informa a Cliente y Proveedor. Si el pago requiere una devolución externa, debe procesarse por el flujo financiero correspondiente.</p></div><div className="ugo-resolution-choice"><button className={favor==='cliente'?'active':''} onClick={()=>setFavor('cliente')}>A favor del cliente</button><button className={favor==='proveedor'?'active':''} onClick={()=>setFavor('proveedor')}>A favor del proveedor</button></div><textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Explicá la decisión, evidencia revisada y motivo..."/><div className="ugo-resolution-actions"><button onClick={()=>setSelected(null)}>Cancelar</button><button className="primary" disabled={!text.trim()||busy} onClick={resolve}>{busy?'Resolviendo…':'Confirmar resolución'}</button></div></div>}
 </div>
}

...
export function AdminDisputesDecisionCenter(){
 ...
 const[messages,setMessages]=useState<any[]>([]),[evidence,setEvidence]=useState<any[]>([]),[detailLoading,setDetailLoading]=useState(false)
 const total=useMemo(()=>disputes.reduce((n:any,d:any)=>n+Number(d.monto_disputado||0),0),[disputes])
 useEffect(()=>{if(!selected){setMessages([]);setEvidence([]);return}let alive=true;setDetailLoading(true);Promise.all([
 ...
 ]).then(([m,e]:any[])=>{if(!alive)return;setMessages(m.data||[]);setEvidence(e.data||[]);setDetailLoading(false)}).catch(()=>{if(alive)setDetailLoading(false)});return()=>{alive=false}},[selected?.id])
 const resolve=async()=>{if(!selected||!text.trim())return;setBusy(true);setNotice(null);try{await resolverDisputa(selected.id,text.trim(),favor);setSelected(null);setText('');setNotice('Disputa resuelta y visible para ambas partes.')}catch(e){setNotice(e instanceof Error?e.message:'No se pudo resolver la disputa.')}finally{setBusy(false)}}
 return <div className="ugo-decision">
  ...
  {loading?<div className="ugo-decision-empty">Cargando disputas…</div>:<div className="ugo-dispute-grid">{disputes.map((d:any)=><article key={d.id} className="ugo-dispute-card"><header><div><span>{String(d.estado||'abierta').replace('_',' ')}</span><h3>Caso #{d.numero||String(d.id).slice(0,8)}</h3></div><strong>{money(d.monto_disputado)}</strong></header><p>{d.motivo||'Sin motivo detallado.'}</p><dl><div><dt>Cliente</dt><dd>{d.clientes?.nombre||'Cliente'}</dd></div><div><dt>Proveedor</dt><dd>{d.proveedores?.nombre||'Proveedor'}</dd></div><div><dt>Servicio</dt><dd>{String(d.servicio_id).slice(0,8)}</dd></div><div><dt>Creada</dt><dd>{when(d.created_at)}</dd></div></dl><button className="primary" onClick={()=>{setSelected(d);setText('');setFavor('cliente')}}>Revisar y resolver</button></article>)}{!disputes.length&&<div className="ugo-decision-empty">✓ No hay disputas abiertas.</div>}</div>}
  {selected&&<div className="ugo-resolution"><div><small>RESOLUCIÓN ADMINISTRATIVA</small><h3>Caso #{selected.numero||String(selected.id).slice(0,8)}</h3><p>La resolución se registra en UGO y se informa a Cliente y Proveedor. Si el pago requiere una devolución externa, debe procesarse por el flujo financiero correspondiente.</p></div>
   <div className="ugo-case-context"><section><small>HILO DEL CASO</small>{detailLoading?<p>Cargando…</p>:messages.length?messages.map((m:any)=><article key={m.id}><div><b>{m.autor_rol==='admin'?'UGO Admin':m.autor_rol==='cliente'?'Cliente':'Proveedor'}</b><span>{when(m.created_at)}</span></div><p>{m.mensaje}</p>{Array.isArray(m.evidencias)&&m.evidencias.length>0&&<em>{m.evidencias.length} evidencia(s) adjunta(s)</em>}</article>):<p>Sin respuestas adicionales todavía.</p>}</section>
   <section><small>EVIDENCIAS DEL SERVICIO</small>{detailLoading?<p>Cargando…</p>:evidence.length?evidence.map((e:any)=><article key={e.id}><div><b>{String(e.tipo||'evidencia').toUpperCase()}</b><span>{when(e.created_at)}</span></div><p>{e.descripcion||'Sin descripción.'}</p><code>{e.storage_path}</code></article>):<p>No hay fotos o documentos registrados para este servicio.</p>}</section></div>
   <div className="ugo-resolution-choice"><button className={favor==='cliente'?'active':''} onClick={()=>setFavor('cliente')}>A favor del cliente</button><button className={favor==='proveedor'?'active':''} onClick={()=>setFavor('proveedor')}>A favor del proveedor</button></div><textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Explicá la decisión, evidencia revisada y motivo..."/><div className="ugo-resolution-actions"><button onClick={()=>setSelected(null)}>Cancelar</button><button className="primary" disabled={!text.trim()||busy} onClick={resolve}>{busy?'Resolviendo…':'Confirmar resolución'}</button></div></div>}
 </div>
}

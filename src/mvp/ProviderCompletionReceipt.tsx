...
type CashDebt={id:string;saldo_pendiente:number;estado:string;referencia_pago?:string|null}

export function ProviderCompletionReceipt({onFindAnother}:Props){
 const[uid,setUid]=useState(''),[payment,setPayment]=useState<ReleasedPayment|null>(null),[debt,setDebt]=useState<CashDebt|null>(null),[open,setOpen]=useState(false),[busy,setBusy]=useState(false)
 ...
 const load=useCallback(async(userId:string)=>{
  ...
  setPayment(row)
  if(!row){setDebt(null);return setOpen(false)}
  if(row.metodo==='efectivo'){const{data:debtRow}=await db.from('deudas_ugo_proveedor').select('id,saldo_pendiente,estado,referencia_pago').eq('pago_id',row.id).maybeSingle();setDebt((debtRow as CashDebt|null)||null)}else setDebt(null)
 ...
 return <div style={{position:'fixed',inset:0,zIndex:13050,background:'rgba(5,10,14,.72)',backdropFilter:'blur(8px)',display:'grid',placeItems:'center',padding:18}}>
  <section style={{width:'min(430px,100%)',background:'#fff',borderRadius:28,padding:22,boxShadow:'0 24px 80px rgba(0,0,0,.35)',color:'#101418'}}>
   ...
   <div style={{textAlign:'center'}}><small style={{fontWeight:900,letterSpacing:1,color:'#258a49'}}>{isCash?'EFECTIVO REGISTRADO':'PAGO LIBERADO EN UGO'}</small><h2 style={{margin:'6px 0 4px'}}>Trabajo aprobado</h2><p style={{margin:0,color:'#657079',fontSize:13}}>{isCash?`El cliente confirmó que te pagó en efectivo el servicio #${service?.numero||''}. Ese dinero ya lo recibiste directamente.`:`El cliente aprobó el servicio #${service?.numero||''}. Tu ganancia está disponible en el saldo UGO.`}</p></div>
   <div style={{margin:'18px 0',padding:16,borderRadius:20,background:'#f7f9fa'}}>
    ...
    <div style={{display:'flex',justifyContent:'space-between',gap:12,marginBottom:10}}><span style={{color:'#657079'}}>{isCash?'Cobrado en efectivo':'Total'}</span><b>{money(payment.monto_bruto,payment.moneda)}</b></div>
    <div style={{display:'flex',justifyContent:'space-between',gap:12,marginBottom:10}}><span style={{color:'#657079'}}>Comisión UGO</span><b>{isCash?'Pendiente ':'- '}{money(payment.comision_ugo,payment.moneda)}</b></div>
    <div style={{height:1,background:'#dde2e5',margin:'12px 0'}}/>
    <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'end'}}><span style={{fontWeight:800}}>{isCash?'Neto después de comisión':'Ganaste'}</span><strong style={{fontSize:27}}>{money(payment.ganancia_proveedor,payment.moneda)}</strong></div>
   </div>
   {isCash&&debt&&<div style={{margin:'-4px 0 16px',padding:12,borderRadius:16,background:debt.estado==='pagado'?'#ecfdf3':'#fff8e8',border:`1px solid ${debt.estado==='pagado'?'#a6f4c5':'#fedf89'}`,fontSize:12,lineHeight:1.5}}><b>{debt.estado==='pagado'?'Comisión UGO saldada':'Tenés una comisión pendiente con UGO'}</b><div>{debt.estado==='pagado'?'No tenés deuda por este servicio.':`Saldo pendiente: ${money(debt.saldo_pendiente,payment.moneda)}. Lo vas a ver también en Ganancias.`}</div>{debt.referencia_pago&&<small>Referencia: {debt.referencia_pago}</small>}</div>}

...
type CashDebt={id:string;pago_id:string;servicio_id:string;proveedor_id:string;monto_servicio:number;comision_ugo:number;monto_pagado_ugo:number;saldo_pendiente:number;moneda:string;estado:string;referencia_pago?:string|null;created_at:string;ambiente:'real'|'demo';proveedor?:{nombre:string}|null;servicio?:{numero:number|string|null}|null}
...
export function AdminFinancePanel({embedded=false}:Props){
 ...
 const load=useCallback(async()=>{
  ...
  const[{data:p,error:pe},{data:w,error:we},{data:d,error:de}]=await Promise.all([
   ...
   db.from('deudas_ugo_proveedor').select('id,pago_id,servicio_id,proveedor_id,monto_servicio,comision_ugo,monto_pagado_ugo,saldo_pendiente,moneda,estado,referencia_pago,created_at,ambiente,proveedor:usuarios!deudas_ugo_proveedor_proveedor_id_fkey(nombre),servicio:servicios!deudas_ugo_proveedor_servicio_id_fkey(numero)').order('created_at',{ascending:false}).limit(500),
  ])
 ...
 const realDebts=useMemo(()=>debts.filter(d=>d.ambiente==='real'),[debts])
 const pendingDebts=useMemo(()=>realDebts.filter(d=>!['pagado','anulado'].includes(d.estado)),[realDebts])
 const totals=useMemo(()=>{
  ...
  const retained=valid.filter(p=>p.estado==='retenido'&&p.metodo!=='efectivo').reduce((a,p)=>a+Number(p.monto_bruto||0),0)
  const releasedProvider=valid.filter(p=>p.estado==='liberado'&&p.metodo!=='efectivo').reduce((a,p)=>a+Number(p.ganancia_proveedor||0),0)
  const digitalFees=valid.filter(p=>p.estado==='liberado'&&p.metodo!=='efectivo').reduce((a,p)=>a+Number(p.comision_ugo||0),0)
  const cashCollected=valid.filter(p=>p.estado==='liberado'&&p.metodo==='efectivo').reduce((a,p)=>a+Number(p.monto_bruto||0),0)
  const cashDebt=pendingDebts.reduce((a,d)=>a+Number(d.saldo_pendiente||0),0)
  const cashFeesCollected=realDebts.filter(d=>d.estado==='pagado').reduce((a,d)=>a+Number(d.monto_pagado_ugo||0),0)
  ...
  return{gmv,retained,releasedProvider,cashCollected,cashDebt,feesCollected:digitalFees+cashFeesCollected,refunded,withdrawn,withdrawalPending,providerBalance:Math.max(0,releasedProvider-withdrawn)}
 },[realPayments,realWithdrawals,pending,pendingDebts,realDebts])
 ...
 async function settleDebt(row:CashDebt){
  ...
  setBusy('');if(error)return setMessage(error.message)
  setMessage('Comisión UGO marcada como saldada y auditada.');await load()
 }
 const body=<div style={{background:'#fff',border:embedded?'0':'1px solid #ddd',borderRadius:22,padding:16,color:'#111'}}>
   ...
   <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:8,marginTop:12}}>
    {[['GMV REAL',totals.gmv],['SALDO UGO PROV.',totals.providerBalance],['EFECTIVO COBRADO',totals.cashCollected],['DEUDA UGO EFECTIVO',totals.cashDebt],['COMISIÓN UGO COBRADA',totals.feesCollected],['RETENIDO DIGITAL',totals.retained],['RETIRADO',totals.withdrawn],['REEMBOLSADO',totals.refunded]].map(([label,value])=><div key={String(label)} style={{padding:11,borderRadius:14,background:'#f6f7f8'}}><small>{label}</small><b style={{display:'block',marginTop:3}}>{money(Number(value))}</b></div>)}
   </div>
   ...
   <h3 style={{fontSize:13,margin:'16px 0 8px'}}>Comisiones UGO por efectivo ({pendingDebts.length}) · {money(totals.cashDebt)}</h3>
   {pendingDebts.length===0&&<div style={{padding:16,textAlign:'center',fontSize:12,opacity:.6}}>No hay comisiones de efectivo pendientes.</div>}
   {pendingDebts.map(d=><div key={d.id} style={{border:'1px solid #fedf89',background:'#fffcf5',borderRadius:14,padding:11,marginBottom:8}}><div style={{display:'flex',justifyContent:'space-between',gap:8}}><div><b>{d.proveedor?.nombre||'Proveedor UGO'}</b><small style={{display:'block',opacity:.65}}>Servicio #{d.servicio?.numero||String(d.servicio_id).slice(0,8)} · cobró {money(d.monto_servicio,d.moneda)} en efectivo · {d.estado}</small></div><strong>{money(d.saldo_pendiente,d.moneda)}</strong></div><input value={debtRefs[d.id]??d.referencia_pago??''} onChange={e=>setDebtRefs(v=>({...v,[d.id]:e.target.value}))} placeholder="Referencia PIX / E2E / transferencia de comisión" style={{width:'100%',boxSizing:'border-box',marginTop:8,padding:9,border:'1px solid #d0d5dd',borderRadius:10}}/><div style={{display:'flex',gap:6,marginTop:8}}>{d.estado==='informado'&&<span style={{flex:1,fontSize:11,color:'#b54708',alignSelf:'center'}}>Proveedor informó este pago. Verificá la referencia antes de conciliar.</span>}<button disabled={!!busy} onClick={()=>void settleDebt(d)} style={{padding:'8px 12px',borderRadius:10,border:0,background:'#111820',color:'#fff',fontWeight:800}}>Confirmar comisión pagada</button></div></div>)}

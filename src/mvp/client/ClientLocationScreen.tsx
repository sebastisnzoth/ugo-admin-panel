...
export function ClientLocationScreen({onBack,onContinue}:{onBack:()=>void;onContinue:()=>void}){
 ...
 const useLocation=()=>{
  ...
  navigator.geolocation.getCurrentPosition(async pos=>{
   try{
    ...
    if(value){setAddress(value);setZone(nextZone);save(value,nextZone,null);void loadQuote(value,nextZone)}else setMessage('No pudimos obtener la dirección. Escribila abajo.')
 ...
 return <main className="ugo-location-screen" aria-label="Dónde es el servicio">
  ...
  <section><h1>¿Dónde es el servicio?</h1><p>Confirmá la dirección</p>
   ...
   <label>Dirección<input value={address} onChange={e=>{setAddress(e.target.value);setQuote(null);setQuoteChecked(false);setMessage('')}} onBlur={()=>{if(address.trim().length>=5)void loadQuote(address,zone)}} placeholder="Rua Apóstolo Paschoal, 123" autoComplete="street-address"/></label>

...
type Draft={address?:string;addressLabel?:string;complement?:string;description?:string;categoryName?:string;categoryId?:string;zone?:string;tariffQuote?:TariffQuote|null}
...
export function ClientLocationScreen({onBack,onContinue}:{onBack:()=>void;onContinue:()=>void}){
 ...
 const choosePlace=(place:SavedPlace)=>{const nextZone=place.barrio||place.ciudad||'';setSelectedPlace(place.id);setAddress(place.direccion);setComplement(place.complemento||'');setZone(nextZone);setMessage('');setQuote(null);setQuoteChecked(false);save(place.direccion,nextZone,null);void loadQuote(place.direccion,nextZone)}
 const useLocation=()=>{
  ...
  navigator.geolocation.getCurrentPosition(async pos=>{
   try{
    ...
    if(value){setSelectedPlace('');setAddress(value);setZone(nextZone);save(value,nextZone,null);try{sessionStorage.setItem('ugo:last-client-location',JSON.stringify({latitude:pos.coords.latitude,longitude:pos.coords.longitude,label:value,at:Date.now()}));if(session)await (supabase as any).from('perfiles_cliente').update({ubicacion:`POINT(${pos.coords.longitude} ${pos.coords.latitude})`}).eq('usuario_id',session.user.id)}catch(error){console.warn('UGO client GPS persistence unavailable',error)}void loadQuote(value,nextZone)}else setMessage('No pudimos obtener la dirección. Escribila abajo.')
 ...
 return <main className="ugo-location-screen" aria-label="Dónde es el servicio">
  ...
  <section><h1>¿Dónde es el servicio?</h1><p>Elegí un lugar guardado o usá tu ubicación actual.</p>
   ...
   <label>Dirección<input value={address} onChange={e=>{setSelectedPlace('');setAddress(e.target.value);setQuote(null);setQuoteChecked(false);setMessage('')}} onBlur={()=>{if(address.trim().length>=5)void loadQuote(address,zone)}} placeholder="Rua Apóstolo Paschoal, 123" autoComplete="street-address"/></label>

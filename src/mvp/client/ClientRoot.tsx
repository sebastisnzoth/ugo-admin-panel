...
import{ClientHugoVoiceOrder}from'./ClientHugoVoiceOrder'
...
export function ClientRoot({demo}:Props){
 ...
 return <ClientOnboardingGate><div className="ugo-client-root"><ClientFlowActionsBridge/>{demo&&<DemoSebastianPaymentBridge/>}<ClientPremiumHome/><ClientGuidedRequest/><ClientHugoVoiceOrder/><ClientPaymentChoice/><ClientGlobalMenu/><NotificationCenter role="client" onOpenNotice={openNotice}/><ClientLiveTracking/><ClientCompletionReview onOpenDispute={flow.actions.openDispute}/><ServiceExpansionPanel role="client"/><DisputeDock role="client" openRequest={flow.screen==='dispute'}/><AppLocationButton role="client"/>{flow.screen==='history'&&<div className="ugo-client-screen-overlay"><div className="ugo-client-history-wrap"><button type="button" onClick={()=>flow.navigate('home')} style={{width:44,height:44,borderRadius:14,border:'1px solid #2d4357',background:'#102335',color:'#f6fbff',fontSize:20,marginBottom:10}} aria-label="Volver">←</button><ServiceHistoryPanel role="client" embedded/></div></div>}{flow.screen==='profile'&&<ClientProfilePanel/>}</div></ClientOnboardingGate>
}

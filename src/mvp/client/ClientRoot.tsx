...
export function ClientRoot({demo}:Props){
 ...
 const closeService=()=>{clearSentinelContext();setSelectedServiceId(null)},detailOpen=Boolean(selectedServiceId),canonical=flow.screen==='home'||flow.screen==='request',showNav=flow.screen!=='request'&&!detailOpen
 return <ClientOnboardingGate><div className={`ugo-client-root ugo-client-screen-${flow.screen}`}><ClientFlowActionsBridge/>{demo&&<DemoSebastianPaymentBridge/>}{flow.screen==='home'&&<ClientHomeScreen/>}{flow.screen==='request'&&<ClientNeedScreen/>}<ClientGlobalMenu/><NotificationCenter role="client" onOpenNotice={openNotice}/>{!canonical&&!detailOpen&&<ClientHugoBridge/>}{!canonical&&!detailOpen&&<ClientPaymentChoice/>}{!canonical&&!detailOpen&&<ClientLiveTracking/>}{!canonical&&!detailOpen&&<ClientCompletionReview onOpenDispute={flow.actions.openDispute}/>}{!canonical&&!detailOpen&&<ClientRatingPrompt/>}{!canonical&&!detailOpen&&<ServiceChat role="client"/>}{!canonical&&!detailOpen&&<DisputeDock role="client" openRequest={flow.screen==='dispute'}/>}{!canonical&&!detailOpen&&<AppLocationButton role="client"/>}{!canonical&&<ClientProviderRadarBridge/>}{flow.screen==='history'&&!detailOpen&&<div className="ugo-client-screen-overlay"><div className="ugo-client-history-wrap"><button type="button" className="ugo-client-activity-back" onClick={()=>flow.navigate('home')} aria-label="Volver al inicio">‹ <span>Inicio</span></button><ServiceHistoryPanel role="client" embedded onOpenService={openService}/></div></div>}{selectedServiceId&&<SentinelErrorBoundary role="client" serviceId={selectedServiceId} checklistCode="CLIENT-ORDER-OPEN" action="client.activity.open_order" severity="P0" title="No pudimos abrir este pedido" onClose={closeService}><ClientServiceDetail serviceId={selectedServiceId} onClose={closeService}/></SentinelErrorBoundary>}{flow.screen==='profile'&&!detailOpen&&<ClientProfilePanel/>}{showNav&&flow.screen!=='home'&&<nav className="ugo-client-persistent-bottom" aria-label="Navegación inferior"><button type="button" onClick={()=>flow.navigate('home')}><span>⌂</span><small>Inicio</small></button><button type="button" onClick={()=>flow.navigate('home')}><span>▦</span><small>Servicios</small></button><button type="button" className={flow.screen==='history'?'active':''} onClick={()=>flow.navigate('history')}><span>▣</span><small>Actividad</small></button><button type="button" className={flow.screen==='profile'?'active':''} onClick={()=>flow.navigate('profile')}><span>♙</span><small>Perfil</small></button></nav>}</div></ClientOnboardingGate>
}

...
export function ClientRoot({demo}:Props){
 ...
 const requestOpen=flow.screen==='request'
 return <ClientOnboardingGate><div className={`ugo-client-root ugo-client-screen-${flow.screen}`}><ClientFlowActionsBridge/>{demo&&<DemoSebastianPaymentBridge/>}<ClientStudioNavbar/>{flow.screen==='home'&&<ClientPremiumHome onOpenService={openService}/>} {requestOpen&&<ClientGuidedRequest key={flow.providerId||'default'}/>} {!requestOpen&&!detailOpen&&<ClientHugoBridge/>}{!requestOpen&&!detailOpen&&<ClientPaymentChoice/>}<ClientGlobalMenu/><NotificationCenter role="client" onOpenNotice={openNotice}/>{!detailOpen&&<ClientLiveTracking/>}{!requestOpen&&!detailOpen&&<ClientCompletionReview onOpenDispute={flow.actions.openDispute}/>}{!requestOpen&&!detailOpen&&<ClientRatingPrompt/>}{!requestOpen&&!detailOpen&&<ServiceChat role="client"/>}{!requestOpen&&!detailOpen&&<DisputeDock role="client" openRequest={flow.screen==='dispute'}/>}{!requestOpen&&!detailOpen&&<AppLocationButton role="client"/>}<ClientProviderRadarBridge/>{flow.screen==='history'&&!detailOpen&&<div className="ugo-client-screen-overlay"><div className="ugo-client-history-wrap"><ServiceHistoryPanel role="client" embedded onOpenService={openService}/></div></div>}{selectedServiceId&&<SentinelErrorBoundary role="client" serviceId={selectedServiceId} checklistCode="CLIENT-ORDER-OPEN" action="client.activity.open_order" severity="P0" title="No pudimos abrir este pedido" onClose={closeService}><ClientServiceDetail serviceId={selectedServiceId} onClose={closeService}/></SentinelErrorBoundary>}{flow.screen==='profile'&&!detailOpen&&<ClientProfilePanel/>}</div></ClientOnboardingGate>
}

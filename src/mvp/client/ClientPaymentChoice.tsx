...
export function ClientPaymentChoice(){
 ...
 const[service,setService]=useState<Service|null>(null),[payment,setPayment]=useState<Payment|null>(null),[cashEnabled,setCashEnabled]=useState(true),[busy,setBusy]=useState<'pix'|'cash'|''>(''),[message,setMessage]=useState('')
 const load=useCallback(async()=>{if(!session){setService(null);setPayment(null);return}const{data:rows}=await supabase.from('servicios').select('*').eq('cliente_id',session.user.id).in('estado',PAYMENT_STATES).order('created_at',{ascending:false}).limit(1);const current=((rows||[])[0]as Service|undefined)||null;setService(current);if(!current){setPayment(null);setCashEnabled(true);return}const moneda=String((current as ServiceWithCurrency).moneda||'BRL');const[{data:p},{data:cashAllowed,error:cashError}]=await Promise.all([supabase.from('pagos').select('id,servicio_id,metodo,estado,pix_copia_cola,pix_qr_code,pix_expira_at,mp_payment_id,pago_externo_id,pix_e2e_id,created_at').eq('servicio_id',current.id).order('created_at',{ascending:false}).limit(1).maybeSingle(),supabase.rpc('cash_payment_enabled',{p_moneda:moneda})]);setPayment((p as Payment|null)||null);setCashEnabled(cashError?true:cashAllowed!==false)},[session,supabase])
 useEffect(()=>{const timer=window.setTimeout(()=>void load(),0);return()=>window.clearTimeout(timer)},[load])
 useEffect(()=>{if(!session)return;const refresh=()=>void load().catch(()=>{});const ch=supabase.channel(`client-payment-choice-${session.user.id}`).on('postgres_changes',{event:'*',schema:'public',table:'pagos',filter:`cliente_id=eq.${session.user.id}`},refresh).on('postgres_changes',{event:'*',schema:'public',table:'servicios',filter:`cliente_id=eq.${session.user.id}`},refresh).subscribe(status=>{if(status==='SUBSCRIBED')refresh()});const onOnline=()=>refresh();const onVisibility=()=>{if(document.visibilityState==='visible')refresh()};window.addEventListener('online',onOnline);document.addEventListener('visibilitychange',onVisibility);return()=>{window.removeEventListener('online',onOnline);document.removeEventListener('visibilitychange',onVisibility);supabase.removeChannel(ch)}},[load,session,supabase])

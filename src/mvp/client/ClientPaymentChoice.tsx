...
type Payment={id:string;servicio_id:string;metodo?:string|null;estado:string;pix_copia_cola?:string|null;pix_qr_code?:string|null;pix_expira_at?:string|null}
const PAYMENT_STATES=['asignado','pago_pendiente']
...
export function ClientPaymentChoice(){
 ...
 const load=useCallback(async()=>{if(!session){setService(null);setPayment(null);return}const{data:rows}=await supabase.from('servicios').select('*').eq('cliente_id',session.user.id).in('estado',PAYMENT_STATES).order('created_at',{ascending:false}).limit(1);const current=((rows||[])[0]as Service|undefined)||null;setService(current);if(!current){setPayment(null);return}const{data:p}=await supabase.from('pagos').select('id,servicio_id,metodo,estado,pix_copia_cola,pix_qr_code,pix_expira_at,created_at').eq('servicio_id',current.id).order('created_at',{ascending:false}).limit(1).maybeSingle();setPayment((p as Payment|null)||null)},[session,supabase])
 ...
 return <section className="ugo-client-payment-choice" aria-label="Elegir forma de pago">
  <header><small>SERVICIO #{service.numero}</small><h2>¿Cómo querés pagar?</h2><p>Elegí una opción sin salir del servicio. UGO mantiene el mismo seguimiento.</p></header>
  <div className="ugo-client-payment-options">
   <button type="button" className={selected==='pix'?'selected':''} onClick={choosePix} disabled={Boolean(busy)}><span>⚡</span><div><b>Pix</b><small>Pago electrónico confirmado dentro de UGO.</small></div><i>{selected==='pix'?'✓':'›'}</i></button>
   <button type="button" className={selected==='efectivo'?'selected':''} onClick={chooseCash} disabled={Boolean(busy)}><span>💵</span><div><b>Efectivo</b><small>Pagás al profesional al finalizar. UGO registra la confirmación.</small></div><i>{selected==='efectivo'?'✓':'›'}</i></button>
  </div>
  ...
  {selected==='efectivo'&&<div className="ugo-client-cash-note"><b>Efectivo elegido</b><span>El profesional confirma la recepción al terminar. UGO no presenta este pago como dinero protegido.</span></div>}

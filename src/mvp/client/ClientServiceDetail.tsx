...
export function ClientServiceDetail({serviceId,onClose}:{serviceId:string;onClose:()=>void}){
 ...
 useEffect(()=>{let alive=true;const resync=()=>{if(alive)void load()};const onVisibility=()=>{if(document.visibilityState==='visible')resync()};window.addEventListener('online',resync);document.addEventListener('visibilitychange',onVisibility);const ch=supabase.channel(`client-service-detail-${serviceId}`).on('postgres_changes',{event:'*',schema:'public',table:'servicios',filter:`id=eq.${serviceId}`},resync).subscribe(status=>{if(status==='SUBSCRIBED')resync();if(status==='CHANNEL_ERROR'||status==='TIMED_OUT')void reportSentinelIncident({eventType:'realtime_subscription_error',message:`Detalle de pedido Realtime: ${status}`,role:'client',severity:'P1',serviceId,action:'client.order.realtime'})});return()=>{alive=false;window.removeEventListener('online',resync);document.removeEventListener('visibilitychange',onVisibility);void supabase.removeChannel(ch)}},[load,serviceId,supabase])
 ...
 return <div className="ugo-client-screen-overlay" role="dialog" aria-modal="true" aria-label="Detalle del pedido"><div className="ugo-client-history-wrap" style={{display:'grid',gap:12}}><button type="button" className="ugo-client-history-back" onClick={onClose} aria-label="Volver a mis pedidos">←</button>
  ...
  {!loading&&!error&&service&&<><section className="ugo-history-panel embedded"><header><div><small>PEDIDO #{service.numero??String(service.id).slice(0,8)}</small><h2>{service.categoria?.emoji||'🧰'} {service.categoria?.nombre||'Servicio UGO'}</h2><p>{service.descripcion||'Sin descripción adicional.'}</p></div><span className={`state-${service.estado}`}>{STATUS_LABELS[service.estado]||service.estado}</span></header><div className="ugo-history-meta"><div><small>{service.programado_para?'PROGRAMADO':'CREADO'}</small><b>{when(service.programado_para,service.created_at)}</b></div><div><small>PROVEEDOR</small><b>{service.proveedor?.nombre||'Todavía sin asignar'}</b></div><div><small>DIRECCIÓN</small><b>{service.direccion_cliente||'Por confirmar'}</b></div><div><small>IMPORTE</small><b>{money(service.tarifa,service.moneda)}</b></div></div>{notice&&<div className="ugo-history-action-notice" role="status">{notice}</div>}{CANCELLABLE.has(service.estado)&&<div className="ugo-history-row-actions"><button type="button" className="ugo-history-cancel-button" disabled={busy} onClick={()=>void cancel()}>{busy?'Cancelando…':'Cancelar este pedido'}</button></div>}</section>
   ...
   <SentinelErrorBoundary role="client" serviceId={service.id} action="client.order.tracking" title="Seguimiento temporalmente no disponible" compact><ClientLiveTracking serviceId={service.id} embedded/></SentinelErrorBoundary>
   <SentinelErrorBoundary role="client" serviceId={service.id} action="client.order.payment" checklistCode="PAYMENT-CLOSE" severity="P0" title="Pago temporalmente no disponible" compact><ClientPaymentChoice serviceId={service.id}/></SentinelErrorBoundary>
   <SentinelErrorBoundary role="client" serviceId={service.id} action="client.order.expansion" title="Cambios del servicio temporalmente no disponibles" compact><ServiceExpansionPanel role="client" serviceId={service.id} compact/></SentinelErrorBoundary>
   <SentinelErrorBoundary role="client" serviceId={service.id} action="client.order.review" title="Cierre temporalmente no disponible" compact><ClientCompletionReview serviceId={service.id}/></SentinelErrorBoundary>

...
export function ClientProviderRadarBridge(){
 ...
 useEffect(()=>{
  ...
  const refresh=async()=>{try{await refreshProviderRadar(supabase,true)}catch(error){if(alive){console.warn('No pudimos sincronizar el radar compartido.',error);if(document.visibilityState==='visible'&&navigator.onLine)report('client_provider_radar_sync_error',error instanceof Error?error.message:'No pudimos sincronizar profesionales online.',error)}}}
  ...
  void refresh()
  const ch=supabase.channel(`client-provider-radar-${session.user.id}-${instanceId}-${channelEpoch}`).on('postgres_changes',{event:'*',schema:'public',table:'perfiles_proveedor'},()=>{void refresh()}).subscribe(status=>{
   if(status==='SUBSCRIBED')void refresh()
   else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'){if(document.visibilityState==='visible'&&navigator.onLine)report('client_provider_radar_realtime_error',`Canal radar: ${status}`);void refresh();reconnect()}
  })
 ...
 },[channelEpoch,instanceId,session,supabase])

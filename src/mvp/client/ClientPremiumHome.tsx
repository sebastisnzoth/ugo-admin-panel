...
export function ClientPremiumHome(){
 ...
 return <main className="ugo-client-premium-home ugo-client-conversational-home">
  ...
  <section className="ugo-client-home-copy ugo-client-home-hugo-copy"><span className="ugo-client-home-kicker">Hugo · {firstName?`acá estoy, ${firstName}`:'tu compañero UGO'}</span><h1>Contame qué necesitás. Yo te ayudo a resolverlo.</h1><p>Hablame o escribime como a un amigo. Yo voy armando el pedido, pregunto sólo lo que falta y te acompaño hasta el final.</p><div className="ugo-client-home-hugo-actions"><button type="button" className="ugo-client-home-talk" onClick={talkToHugo}><b>⌁</b><span>Hablar con Hugo</span><em>●</em></button><button type="button" className="ugo-client-home-search" onClick={()=>writeToHugo()}><b>✎</b><span>Escribirle a Hugo</span><em>→</em></button></div></section>
  <section className="ugo-client-home-categories" aria-label="Atajos de servicios">{categories.map(c=><button key={c.id} type="button" onClick={()=>writeToHugo(`Necesito ${c.nombre}`,true)}><span>{c.emoji||'🧰'}</span><small>{c.nombre}</small></button>)}</section>

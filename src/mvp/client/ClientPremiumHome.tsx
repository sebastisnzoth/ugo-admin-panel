...
export function ClientPremiumHome(){
 ...
 return <main className="ugo-client-premium-home">
  ...
  <section className="ugo-client-home-sheet"><div className="ugo-client-home-handle"/><strong>Un pedido. Un profesional. Sin vueltas.</strong><p>Describí lo que necesitás, confirmá dónde y cuándo, y UGO se ocupa de encontrar opciones.</p><button type="button" onClick={()=>openRequest()}>Pedir un servicio <span>→</span></button></section>
  <nav aria-label="Navegación principal"><button className="active" type="button" aria-current="page">⌂<span>Inicio</span></button><button type="button" onClick={()=>flow.actions.openHistory()}>▦<span>Servicios</span></button><button type="button" onClick={()=>flow.actions.openHistory()}>◷<span>Actividad</span></button><button type="button" onClick={()=>flow.actions.openProfile()}>◯<span>Perfil</span></button></nav>
 </main>
}

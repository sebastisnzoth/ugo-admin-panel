...
export function ClientPremiumHome(){
 ...
 return <main className="ugo-client-premium-home ugo-client-conversational-home">
  ...
  <section className="ugo-client-home-copy ugo-client-home-hugo-copy"><span className="ugo-client-home-kicker">{firstName?'Hola, '+firstName:'HOLA'}</span><h1>¿Qué necesitás resolver?</h1><p>Contale el problema a Hugo. UGO arma el pedido y busca al profesional indicado.</p><div className="ugo-client-home-hugo-actions"><button type="button" className="ugo-client-home-talk" onClick={talkToHugo}><b>⌁</b><span>Hablar con Hugo</span><em>●</em></button><button type="button" className="ugo-client-home-search" onClick={()=>writeToHugo()}><b>✎</b><span>Escribir el problema</span><em>→</em></button></div></section>
  <section className="ugo-client-home-categories" aria-label="Elegir servicio"><div className="ugo-client-categories-head"><strong>Servicios</strong><small>Elegí uno o contalo con tus palabras</small></div>{categories.map(c=><button key={c.id} type="button" onClick={()=>writeToHugo('Necesito '+c.nombre,true)}><span>{c.emoji||'🧰'}</span><small>{c.nombre}</small></button>)}</section>

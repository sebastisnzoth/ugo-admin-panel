...
export function ClientPremiumHome(){
 ...
 return <main className="ugo-client-premium-home">
  ...
  <section className="ugo-client-home-copy ugo-client-home-hugo-copy"><span className="ugo-client-home-kicker">Hugo · tu compañero UGO</span><h1>Contame qué necesitás. Yo te ayudo a resolverlo.</h1><p>Hablame como a un amigo. Yo armo el pedido, busco opciones y te muestro qué conviene.</p><div className="ugo-client-home-hugo-actions"><button type="button" className="ugo-client-home-talk" onClick={talkToHugo}><b>⌁</b><span>Hablar con Hugo</span><em>●</em></button><button type="button" className="ugo-client-home-search" onClick={()=>openRequest()}><b>⌕</b><span>Escribir pedido</span><em>→</em></button></div></section>
  <section className="ugo-client-home-categories" aria-label="Atajos de servicios">{categories.map(c=><button key={c.id} type="button" onClick={()=>openRequest(c)}><span>{c.emoji||'🧰'}</span><small>{c.nombre}</small></button>)}</section>
  <section className="ugo-client-home-map" aria-label="Profesionales cerca"><div ref={mapEl}/><div className="ugo-client-home-map-shade"/><div className="ugo-client-home-map-label"><span>●</span><div><b>Hugo busca cerca tuyo</b><small>{place}</small></div></div></section>
  <section className="ugo-client-home-sheet"><div className="ugo-client-home-handle"/><strong>Un pedido. Un profesional. Sin vueltas.</strong><p>Vos contame el problema. Hugo pregunta sólo lo que falta, compara las opciones reales y te acompaña hasta que apruebes el trabajo.</p><button type="button" onClick={talkToHugo}>Decí “Hola Hugo” <span>⌁</span></button></section>

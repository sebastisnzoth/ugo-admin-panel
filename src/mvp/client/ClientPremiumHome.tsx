...
export function ClientPremiumHome(){
 ...
 return <main className="ugo-client-premium-home">
  ...
  <nav aria-label="Navegación principal"><button className={flow.screen==='home'?'active':''} type="button" aria-current={flow.screen==='home'?'page':undefined} onClick={()=>flow.navigate('home')}>⌂<span>Inicio</span></button><button className={flow.screen==='history'?'active':''} type="button" onClick={()=>flow.navigate('history')}>▦<span>Servicios</span></button><button className={flow.screen==='history'?'active':''} type="button" onClick={()=>flow.navigate('history')}>◷<span>Actividad</span></button><button className={flow.screen==='profile'?'active':''} type="button" onClick={()=>flow.navigate('profile')}>◯<span>Perfil</span></button></nav>
 </main>
}

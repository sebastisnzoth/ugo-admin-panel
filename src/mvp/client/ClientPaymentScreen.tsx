...
export function ClientPaymentScreen({onBack,onConfirm}:{onBack:()=>void;onConfirm:()=>void}){
 ...
 useEffect(()=>{if(!key||!session)return;let alive=true,hasDraft=false;try{const d=JSON.parse(sessionStorage.getItem(key)||'{}');if(d.paymentMethod==='cash'||d.paymentMethod==='pix'){setMethod(d.paymentMethod);hasDraft=true}}catch{}if(hasDraft)return()=>{alive=false};void(async()=>{try{const{data}=await supabase.from('preferencias_pago_cliente').select('metodo').eq('usuario_id',session.user.id).maybeSingle();if(alive)setMethod(data?.metodo==='pix'?'pix':'cash')}catch{if(alive)setMethod('cash')}})();return()=>{alive=false}},[key,session,supabase])

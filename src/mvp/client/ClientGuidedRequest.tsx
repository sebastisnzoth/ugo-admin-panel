...
type HugoDraftEvent={step?:string;categoryHint?:string|null;categoryId?:string|null;categorySlug?:string|null;description?:string|null;address?:string|null;amount?:number|null;urgent?:boolean}
...
export function ClientGuidedRequest(){
 ...
 const applyCategory=useCallback(async(category:Category,text:string,urgent:boolean)=>{const amount=await estimate(category.id);setDraft(v=>({...v,description:text||v.description,categoryId:category.id,categoryName:category.nombre,categorySlug:category.slug,urgent,amount:amount??v.amount}));return category},[estimate])
 const infer=useCallback(async(text:string,hint?:string|null,urgent=false)=>{let category=hint?categoryByHint(categories,hint):null;if(!category){const intent=parseClientIntent(text,categories);category=categories.find(c=>c.id===intent.categoryId)||null;urgent=urgent||intent.urgency}if(category)return applyCategory(category,text,urgent);setDraft(v=>({...v,description:text||v.description,urgent}));return null},[applyCategory,categories])

 useEffect(()=>{if(!ready)return;const onIntent=(event:Event)=>{const d=(event as CustomEvent<{text?:string;description?:string|null;categoryHint?:string|null;urgent?:boolean}>).detail||{};const text=String(d.description||d.text||'').trim();if(!text&&!d.categoryHint)return;void infer(text,d.categoryHint,Boolean(d.urgent));if(step==='idle')setStep('need')};const onDraft=(event:Event)=>{const d=(event as CustomEvent<HugoDraftEvent>).detail||{};const category=(d.categoryId&&categories.find(c=>c.id===d.categoryId))||(d.categoryHint?categoryByHint(categories,d.categoryHint):null);setDraft(v=>({...v,description:d.description??v.description,address:d.address||v.address,amount:d.amount??v.amount,urgent:d.urgent??v.urgent,categoryId:category?.id||d.categoryId||v.categoryId,categoryName:category?.nombre||v.categoryName,categorySlug:category?.slug||d.categorySlug||v.categorySlug}));if(step==='idle')setStep('need')};const onOpen=()=>{if(hasActive){setMessage('Ya tenés un servicio en curso. Podés seguirlo desde Inicio.');return}setMessage('');setStep('need')};window.addEventListener('ugo:hugo-ai-intent',onIntent as EventListener);window.addEventListener('ugo:hugo-order-draft',onDraft as EventListener);window.addEventListener(UGO_CLIENT_GUIDED_REQUEST_OPEN,onOpen);return()=>{window.removeEventListener('ugo:hugo-ai-intent',onIntent as EventListener);window.removeEventListener('ugo:hugo-order-draft',onDraft as EventListener);window.removeEventListener(UGO_CLIENT_GUIDED_REQUEST_OPEN,onOpen)}},[categories,hasActive,infer,ready,step])
 ...
 function editReview(target:'need'|'photo'|'when'){setMessage('');setStep(target)}
 async function retryMatching(){if(!activeServiceId||retrying)return;setRetrying(true);setMessage('Reintentando búsqueda con profesionales disponibles…');try{const result=await getDispatchProvider().start({serviceId:activeServiceId,category:draft.categorySlug||draft.categoryId});const count=Array.isArray(result.raw)?result.raw.length:result.providerId?1:0;setMessage(count?`UGO volvió a avisar a ${count} profesional${count===1?'':'es'}. Seguimos esperando aceptación.`:'Todavía no hay profesionales disponibles. Tu solicitud sigue activa y podés volver a intentar.')}catch(e){setMessage(e instanceof Error?e.message:'No pudimos reintentar ahora. Tu solicitud sigue guardada.')}finally{setRetrying(false)}}
 async function cancelMatching(){const ok=await flow.actions.cancelService();if(ok){setStep('idle');setHasActive(false);setActiveServiceId('');setMessage('');flow.navigate('home')}else setMessage('No pudimos cancelar desde esta pantalla. Probá desde Servicios.')}

 async function submit(){
  if(!session||!draft.categoryId||draft.description.trim().length<5)return
  setBusy(true);setMessage('')
  try{
   const metadata={source:'hugo-guided-request',...(photoCount>0?{request_draft_id:draftId}:{}),requested_when:draft.when,scheduled_at:draft.scheduleAt||null,estimated_tariff:draft.amount,demo:false}
   const{data,error}=await supabase.from('servicios').insert({cliente_id:session.user.id,categoria_id:draft.categoryId,estado:'buscando',descripcion:draft.description.trim(),direccion_cliente:draft.address.trim()||null,tarifa:draft.amount,urgencia:draft.urgent,metadata}).select('id').single()
 ...
 return <section className="ugo-guided-request" aria-label="Solicitud guiada por Hugo"><header><button type="button" className="ugo-guided-back" onClick={()=>step==='need'?setStep('idle'):step==='photo'?setStep('need'):step==='when'?setStep('photo'):step==='review'?setStep('when'):setStep('idle')} aria-label="Volver">←</button><div><small>{Math.min(progress,4)} de 4</small><div className="ugo-guided-progress"><i style={{width:`${Math.min(progress,4)*25}%`}}/></div></div><button type="button" className="ugo-guided-orb" onClick={askHugo} aria-label="Hablar con Hugo">⌁</button></header>
  {step==='need'&&<div className="ugo-guided-step"><p className="ugo-guided-kicker">Hugo te ayuda</p><h2>¿Qué necesitás?</h2><p>Podés hablar o escribir. Hugo va completando este pedido sin perder lo que ya le dijiste.</p><textarea autoFocus value={draft.description} onChange={e=>setDraft(v=>({...v,description:e.target.value}))} placeholder="Ej: Necesito podar el jardín y retirar las ramas"/><button type="button" className="ugo-guided-voice" onClick={askHugo}>🎙 Hablar o escribir con Hugo</button>{selectedCategory&&<div className="ugo-guided-understood"><span>Entendí</span><b>{selectedCategory.emoji} {selectedCategory.nombre}</b><small>{draft.description}</small></div>}<div className="ugo-guided-categories" aria-label="Todas las categorías disponibles">{categories.map(c=><button type="button" key={c.id} className={c.id===draft.categoryId?'selected':''} onClick={()=>void chooseCategory(c)}>{c.emoji||'🧰'} {c.nombre}</button>)}</div><button type="button" className="ugo-guided-primary" onClick={()=>void confirmNeed()}>Sí, es eso</button></div>}
  {step==='photo'&&<div className="ugo-guided-step"><p className="ugo-guided-kicker">Foto opcional</p><h2>¿Una foto ayudaría?</h2><p>Si el problema se entiende mejor viéndolo, agregá una foto. Si no hace falta, podés seguir.</p><ClientRequestEvidence draftId={draftId} inline onBusyChange={setPhotoBusy} onCountChange={setPhotoCount}/><button type="button" className="ugo-guided-primary" disabled={photoBusy} onClick={()=>setStep('when')}>{photoBusy?'Subiendo foto…':photoCount?'Usar esta foto':'Continuar sin foto'}</button></div>}
  ...
  {step==='review'&&<div className="ugo-guided-step"><p className="ugo-guided-kicker">Hugo entendió esto</p><h2>Revisá y confirmá</h2><p>Si algo no está bien, cambialo acá sin empezar de nuevo.</p><div className="ugo-guided-summary"><div><span>Servicio</span><b>{selectedCategory?.emoji} {draft.categoryName}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('need')}>Editar</button></div><div><span>Necesidad</span><b>{draft.description}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('need')}>Editar</button></div><div><span>Cuándo</span><b>{draft.when==='ahora'?'Ahora':draft.when==='hoy'?'Hoy':draft.scheduleAt?new Date(draft.scheduleAt).toLocaleString():'A programar'}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('when')}>Editar</button></div><div><span>Dónde</span><b>{draft.address}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('when')}>Editar</button></div><div><span>Fotos</span><b>{photoCount?`${photoCount} adjunta${photoCount===1?'':'s'}`:'Sin foto · opcional'}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('photo')}>Editar</button></div><div><span>Precio</span><b>{draft.amount!=null?`Referencia actual: R$ ${draft.amount.toFixed(0)}`:'A confirmar según el trabajo y el profesional'}</b></div></div><button type="button" className="ugo-guided-primary" onClick={submit} disabled={busy}>{busy?'Buscando…':'Encontrar profesionales'}</button></div>}
  {step==='matching'&&<div className="ugo-guided-step ugo-guided-matching"><div className="ugo-guided-pulse">⌁</div><p className="ugo-guided-kicker">Búsqueda activa · {formatTimer(matchingSeconds)}</p><h2>Estamos buscando tu profesional</h2><p>{message||'UGO está consultando profesionales disponibles cerca tuyo. No hace falta quedarse en esta pantalla.'}</p><div className="ugo-guided-search-actions"><button type="button" className="ugo-guided-primary" onClick={()=>{setStep('idle');flow.navigate('home')}}>Seguir usando UGO</button><button type="button" className="ugo-guided-voice" onClick={()=>void retryMatching()} disabled={retrying}>{retrying?'Reintentando…':'Reintentar búsqueda'}</button><button type="button" className="ugo-guided-cancel" onClick={()=>void cancelMatching()}>Cancelar solicitud</button></div>{matchingSeconds>=45&&<small>Seguimos buscando. Si no aparece nadie, podés reintentar sin crear otro pedido.</small>}</div>}

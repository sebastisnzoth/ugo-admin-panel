...
export function ClientGuidedRequest(){
 ...
 return <section className="ugo-guided-request" aria-label="Solicitud guiada por Hugo"><header><button type="button" className="ugo-guided-back" onClick={()=>step==='need'?setStep('idle'):step==='photo'?setStep('need'):step==='when'?setStep('photo'):step==='review'?setStep('when'):setStep('idle')} aria-label="Volver">←</button><div><small>{Math.min(progress,4)} de 4</small><div className="ugo-guided-progress"><i style={{width:`${Math.min(progress,4)*25}%`}}/></div></div><button type="button" className="ugo-guided-orb" onClick={askHugo} aria-label="Hablar con Hugo">⌁</button></header>
  ...
  {step==='review'&&<div className="ugo-guided-step"><p className="ugo-guided-kicker">Hugo entendió esto</p><h2>Revisá y confirmá</h2><p>Si algo no está bien, cambialo acá sin empezar de nuevo.</p><div className="ugo-guided-summary"><div><span>Servicio</span><b>{selectedCategory?.emoji} {draft.categoryName}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('need')} aria-label="Editar servicio y necesidad">Editar</button></div><div><span>Necesidad</span><b>{draft.description}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('need')} aria-label="Editar descripción de la necesidad">Editar</button></div><div><span>Cuándo</span><b>{draft.when==='ahora'?'Ahora':draft.when==='hoy'?'Hoy':draft.scheduleAt?new Date(draft.scheduleAt).toLocaleString():'A programar'}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('when')} aria-label="Editar fecha y hora">Editar</button></div><div><span>Dónde</span><b>{draft.address}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('when')} aria-label="Editar dirección">Editar</button></div><div><span>Fotos</span><b>{photoCount?`${photoCount} adjunta${photoCount===1?'':'s'}`:'Sin foto · opcional'}</b><button type="button" className="ugo-guided-edit" onClick={()=>editReview('photo')} aria-label="Editar fotos">Editar</button></div><div><span>Precio</span><b>{draft.amount!=null?`Referencia actual: R$ ${draft.amount.toFixed(0)}`:'A confirmar según el trabajo y el profesional'}</b></div></div><button type="button" className="ugo-guided-primary" onClick={submit} disabled={busy}>{busy?'Buscando…':'Encontrar profesionales'}</button></div>}

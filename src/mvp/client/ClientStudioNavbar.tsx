...
export function ClientStudioNavbar(){
 ...
 const recados=()=>{setNotice('Recados se habilitará cuando el módulo real esté conectado.');window.setTimeout(()=>setNotice(''),3200)}
 const firstName=profile?.nombre?.split(' ')[0]||'Usuario'
 return <>
  <header className="ugo-studio-navbar" data-ugo-source="UGO-PRODUCCION/Navbar">
   <div className="ugo-studio-nav-left">
    ...
    <nav aria-label="Navegación principal UGO">
     <button type="button" className={['search','provider','request'].includes(flow.screen)?'active':''} onClick={services}><span className="material-symbols-outlined">grid_view</span><span>Servicios</span></button>
     <button type="button" onClick={recados}><span className="material-symbols-outlined">inventory_2</span><span>Recados</span></button>
     <button type="button" onClick={()=>window.location.assign(`${window.location.pathname}?app=provider`)}><span className="material-symbols-outlined">handyman</span><span className="ugo-studio-pro-label">Trabajá con UGO</span><span className="ugo-studio-pro-short">Pro</span></button>
    </nav>
   </div>
  ...
  {notice&&<div className="ugo-studio-navbar-notice" role="status">{notice}</div>}
 </>
}

...
export function ClientHomeScreen({onOpenService}:Props){
 ...
 const[place,setPlace]=useState('Florianópolis'),[providers,setProviders]=useState<ProviderRadarRow[]>([]),[radarError,setRadarError]=useState(''),[userPos,setUserPos]=useState<[number,number]>(FLORIPA)
 ...
 useEffect(()=>{if(!session)return;navigator.geolocation?.getCurrentPosition(pos=>setUserPos([pos.coords.longitude,pos.coords.latitude]),()=>{},{enableHighAccuracy:true,timeout:7000,maximumAge:60000})},[session])
 ...
 return <main className="ugo-home-screen" aria-label="Inicio UGO Cliente">
  <section className="ugo-home-layout">
   <div className="ugo-home-hero">
    ...
    {orders.length>0&&<div className="ugo-home-active-orders" aria-label="Pedidos activos">{orders.map(order=><button type="button" key={order.id} className={order.proveedor_id?'is-assigned':'is-searching'} onClick={()=>openOrder(order.id)}><span className="ugo-home-order-icon">{order.proveedor_id?'✓':'⌁'}</span><span className="ugo-home-order-copy"><small>PEDIDO #{order.numero??String(order.id).slice(0,8)}</small><b>{STATE_LABEL[order.estado]||'Pedido activo'}</b><strong>{order.proveedor_id?(order.proveedor?.nombre||'Profesional UGO'):(order.categoria?.nombre||'UGO está buscando')}</strong><em>{order.categoria?.emoji||'🧰'} {order.categoria?.nombre||'Servicio'} · Abrir pedido →</em></span></button>)}</div>}
    <div className="ugo-home-search-wrap">
     <label className="ugo-home-search"><span>⌕</span><input ref={searchRef} value={query} onChange={e=>{setQuery(e.target.value);setShowAll(false)}} placeholder="¿Qué servicio necesitás?"/>{query&&<button type="button" className="ugo-home-search-clear" aria-label="Limpiar búsqueda" onClick={e=>{e.preventDefault();setQuery('');setShowAll(false);window.setTimeout(()=>searchRef.current?.focus(),0)}}>×</button>}</label>
   ...
   <aside className="ugo-home-map-card" aria-label="Mapa de profesionales cerca">
    ...
    <div className="ugo-home-map">
     ...
     <div className="ugo-home-map-label"><span>⌖</span><div><b>Tu ubicación</b><small>{place}</small></div></div>

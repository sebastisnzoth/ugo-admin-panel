...
export function AdminPhase2(){
 ...
 return <div className={`ugo-admin2${section==='home'?' ugo-admin-stitch':''}`}>
  ...
  <main className={`ugo-admin2-main${hugoVoiceActive?' hugo-voice-active':''}`}><header><div><small>UGO · {isSuperAdmin?'SUPER ADMIN':'ADMIN'}</small><h1>{title}</h1></div><button className="ugo-admin2-refresh" onClick={()=>void load()} disabled={loading}>{loading?'Actualizando…':'↻ Actualizar'}</button></header>
  ...
  {section!=='superadmin'&&<ConversationalOrb metrics={metrics} role="admin" section={hugoSection} onVoiceActiveChange={setHugoVoiceActive}/>} 
 </div>
}

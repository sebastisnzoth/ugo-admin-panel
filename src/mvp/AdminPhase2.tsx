...
import{
 AdminOverviewNative,AdminMapNative,AdminServicesNative,AdminAlertsNative,AdminDisputesNative,AdminScoutNative,
...
export function AdminPhase2(){
 ...
 return <div className="ugo-admin2">
  ...
  <main className="ugo-admin2-main"><header><div><small>UGO · ADMIN</small><h1>{title}</h1></div><button className="ugo-admin2-refresh" onClick={()=>load()} disabled={loading}>{loading?'Actualizando…':'↻ Actualizar'}</button></header>
   ...
   {operationView==='overview'&&nativeWrap(<AdminOverviewNative/>)}{operationView==='map'&&nativeWrap(<AdminMapNative/>)}{operationView==='services'&&nativeWrap(<AdminServicesNative/>)}{operationView==='alerts'&&nativeWrap(<AdminAlertsNative/>)}{operationView==='disputes'&&nativeWrap(<AdminDisputesNative/>)}{operationView==='scout'&&nativeWrap(<AdminScoutNative/>)}{operationView==='history'&&<div className="ugo-admin2-history"><ServiceHistoryPanel role="admin" embedded/></div>}{operationView==='messages'&&<div className="ugo-admin2-inline-tool ugo-admin2-messages"><WhatsAppAdminInbox/></div>}</section>}

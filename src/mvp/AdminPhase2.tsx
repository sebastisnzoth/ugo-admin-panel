...
type OperationView='live'|'history'|'messages'
...
export function AdminPhase2(){
 const[section,setSection]=useState<Section>('home'),[operationView,setOperationView]=useState<OperationView>('live'),[metrics,setMetrics]=useState<Metrics>(empty),[loading,setLoading]=useState(true),[updated,setUpdated]=useState<Date|null>(null)
  ...
  <main className="ugo-admin2-main">
   ...
   {section==='home'&&<div className="ugo-admin2-home">
    ...
    <section className="ugo-admin2-priority">
     ...
     <button onClick={()=>{setSection('operations');setOperationView('live')}}><span>Operación en vivo</span><strong>{metrics.active}</strong><small>Servicios activos ahora</small></button>
    </section>
   ...
   {section==='operations'&&<section className="ugo-admin2-section">
    ...
    <div className="ugo-admin2-operations-menu" role="tablist" aria-label="Menú de operaciones">
     <button role="tab" aria-selected={operationView==='live'} className={operationView==='live'?'active':''} onClick={()=>setOperationView('live')}><b>◎</b><span>En vivo</span><small>Mapa, servicios y estados</small>{metrics.active>0&&<em>{metrics.active}</em>}</button>
     <button role="tab" aria-selected={operationView==='history'} className={operationView==='history'?'active':''} onClick={()=>setOperationView('history')}><b>▤</b><span>Historial</span><small>Pedidos y trabajos registrados</small></button>
     <button role="tab" aria-selected={operationView==='messages'} className={operationView==='messages'?'active':''} onClick={()=>setOperationView('messages')}><b>◌</b><span>Mensajes</span><small>WhatsApp y atención operativa</small></button>
    </div>
    <div className="ugo-admin2-operation-context"><small>{operationView==='live'?'OPERACIÓN EN VIVO':operationView==='history'?'HISTORIAL DE SERVICIOS':'COMUNICACIONES'}</small><strong>{operationView==='live'?'Qué está pasando ahora':operationView==='history'?'Trazabilidad completa de UGO':'Conversaciones operativas'}</strong></div>
    {operationView==='live'&&<div className="ugo-admin2-legacy"><AdminPanelBridge/></div>}

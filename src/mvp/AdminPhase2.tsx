...
export function AdminPhase2(){
 ...
 return <div className={`ugo-admin2${section==='home'?' ugo-admin-stitch':''}`}>
  <aside className="ugo-admin2-sidebar"><div className="ugo-admin2-brand"><span>U.GO</span><small>CONTROL CENTER</small></div><nav aria-label="Navegación Admin">
   ...
   <button aria-current={current(section==='superadmin')} className={section==='superadmin'?'active':''} onClick={()=>setSection('superadmin')}><b>◉</b><span>Super Admin</span></button>
  </nav><div className={`ugo-admin2-status${metricsError?' degraded':''}`}><i/>{metricsError?'Datos degradados':'Sistema operativo'}<small>{metricsError?'Reintentar actualización':updated?`Actualizado ${updated.toLocaleTimeString('es-AR',{hour:'2-digit',minute:'2-digit'})}`:'Sincronizando…'}</small></div></aside>
  <main className="ugo-admin2-main"><header><div><small>UGO · ADMIN</small><h1>{title}</h1></div><button className="ugo-admin2-refresh" onClick={()=>void load()} disabled={loading}>{loading?'Actualizando…':'↻ Actualizar'}</button></header>

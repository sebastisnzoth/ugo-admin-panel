...
function RecoveryGate({role,children}:{role:'client'|'provider';children:React.ReactNode}){
 ...
 return <main className="mvp-auth-page"><section className="mvp-auth-card"><div className="mvp-mini-orb"/><div className="mvp-kicker">U.G.O. · {role==='client'?'CLIENTE':'PROVEEDOR'}</div><h1>{phase==='checking'?'Validando enlace…':phase==='error'?'Enlace no válido':'Elegí una contraseña nueva'}</h1>{phase==='checking'?<p>Estamos verificando tu enlace seguro.</p>:phase==='error'?<><p>{message}</p><button className="mvp-primary" onClick={()=>window.location.replace(`${window.location.pathname}?app=${role}`)}>Volver a ingresar</button></>:<form onSubmit={save}><p>Usá al menos 8 caracteres para proteger tu cuenta.</p><label>Nueva contraseña<input type="password" minLength={8} autoComplete="new-password" value={password} onChange={e=>setPassword(e.target.value)} required/></label>{message&&<div className="mvp-form-notice">{message}</div>}<button className="mvp-primary" disabled={busy}>{busy?'Guardando…':'Guardar contraseña'}</button></form>}</section></main>
}

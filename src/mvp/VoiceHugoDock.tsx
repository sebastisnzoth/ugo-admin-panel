...
function primeHugoSpeech(){
 ...
 resumeHugoSpeech()
 if(!mobileVoiceBrowser()||hugoSpeechPrimed)return
 try{
  const primer=new SpeechSynthesisUtterance('\u00a0')
  primer.lang=(navigator.language||'es-AR').toLowerCase().startsWith('pt')?'pt-BR':'es-AR'
  primer.volume=.01
  primer.rate=1
  hugoSpeechPrimer=primer
  const release=()=>{hugoSpeechPrimer=null}
  primer.onend=release
  primer.onerror=release
  window.speechSynthesis.speak(primer)
  hugoSpeechPrimed=true
 }catch{/* browser sin salida TTS */}
}
...
export function VoiceHugoDock({role,accessToken,service,availableOffers=0,mode='dock',draftContext='',paymentStatus='none',clientActions,onIntent}:Props){
 ...
 const toggleVoice=()=>{primeHugoSpeech();if(voice.active)voice.disconnect();else void voice.connect()}
 useEffect(()=>{
  if(!mobileVoiceBrowser()||typeof window==='undefined'||!window.speechSynthesis)return
  const synth=window.speechSynthesis
  const resume=()=>{if(document.visibilityState!=='hidden')resumeHugoSpeech()}
  document.addEventListener('visibilitychange',resume)
  synth.addEventListener?.('voiceschanged',resume)
  return()=>{document.removeEventListener('visibilitychange',resume);synth.removeEventListener?.('voiceschanged',resume)}
 },[])
 useEffect(()=>{
  if(!mobileVoiceBrowser()||!voice.assistantTranscript||typeof window==='undefined'||!window.speechSynthesis)return
  const timers=[0,180,650].map(delay=>window.setTimeout(()=>resumeHugoSpeech(),delay))
  return()=>timers.forEach(timer=>window.clearTimeout(timer))
 },[voice.assistantTranscript])
 useEffect(()=>{
  if(role!=='provider')return
  const handler=(event:Event)=>{event.preventDefault();primeHugoSpeech();setOpen(true)}

...
import { useProviderHugoVoice } from './useProviderHugoVoice'
import { ProviderGlobalMenu } from './ProviderGlobalMenu'
...
type DemandContext={hours:number;total:number;top?:{label?:string;pedidos?:number}|null}
...
const PROVIDER_CONVERSATION_POLICY=[
 ...
 '10) para preguntas de demanda usá solamente el RESUMEN DE DEMANDA REAL del contexto; si dice sin datos suficientes, decilo claramente;',
 '11) respondé normalmente en una o dos frases cortas.'
].join(' ')

export function VoiceHugoDock({role,accessToken,service,availableOffers=0,mode='dock',draftContext=''}:Props){
 const[open,setOpen]=useState(false),[demand,setDemand]=useState<DemandContext|null>(()=>role==='provider'?((window as any).__UGO_PROVIDER_DEMAND__||null):null)
 ...
 const demandText=useMemo(()=>{if(role!=='provider')return'';if(!demand)return'RESUMEN DE DEMANDA REAL: todavía no fue cargado el mapa de demanda.';if(!demand.total||!demand.top)return`RESUMEN DE DEMANDA REAL: sin datos suficientes en las últimas ${demand.hours||24} horas.`;return`RESUMEN DE DEMANDA REAL: ${demand.total} pedidos agregados en ${demand.hours||24} horas. Zona de mayor actividad: ${demand.top.label||'zona aproximada'}, con ${Number(demand.top.pedidos||0)} pedidos agregados.`},[demand,role])
 const context=useMemo(()=>[`Rol: ${role==='client'?'cliente':'proveedor'}`,service?`Servicio #${service.numero}`:'Sin servicio activo',service?`Estado: ${STATUS_LABELS[service.estado]||service.estado}`:'',service?.descripcion?`Descripción: ${service.descripcion}`:'',service?.direccion_cliente?`Dirección: ${service.direccion_cliente}`:'',service?.tarifa!=null?`Tarifa: ${service.moneda||'BRL'} ${service.tarifa}`:'',service?.proveedor?.nombre?`Proveedor: ${service.proveedor.nombre}`:'',availableOffers?`Ofertas pendientes: ${availableOffers}`:'',role==='provider'?PROVIDER_CONVERSATION_POLICY:'',role==='provider'?`PRÓXIMO PASO VALIDADO: ${nextStep}`:'',demandText,draftContext?`MEMORIA DEL PEDIDO: ${draftContext}`:'',`Mensaje operativo actual: ${text}`].filter(Boolean).join(' | '),[availableOffers,role,service,text,draftContext,nextStep,demandText])
 const genericVoice=useHugoVoice({role,accessToken,context})
 const providerVoice=useProviderHugoVoice({accessToken,context,service,availableOffers})
 const voice=role==='provider'?providerVoice:genericVoice
 ...
 useEffect(()=>{if(role!=='provider')return;const onDemand=(event:Event)=>setDemand((event as CustomEvent<DemandContext>).detail||null);window.addEventListener('ugo:provider-demand-updated',onDemand);return()=>window.removeEventListener('ugo:provider-demand-updated',onDemand)},[role])
 useEffect(()=>{if(role!=='provider'||!voice.userTranscript)return;const q=voice.userTranscript.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');if(/donde hay mas trabajo|donde hay trabajo|mapa de demanda|zona de demanda|onde tem mais trabalho|onde ha mais trabalho|mapa de demanda/.test(q))window.dispatchEvent(new Event('ugo:open-provider-demand'))},[role,voice.userTranscript])

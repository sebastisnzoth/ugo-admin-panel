...
export function ClientCashPaymentOption(){
 ...
 async function chooseCash(){setBusy(true);setMessage('');try{const{error}=await supabase.rpc('seleccionar_pago_efectivo',{p_servicio_id:service.id});if(error)throw error;setMessage('Efectivo seleccionado. UGO registra el cobro, pero no custodia el dinero. Después de aprobar el trabajo, vos confirmás en UGO que entregaste el efectivo.');await load()}catch(e){setMessage(e instanceof Error?e.message:'No se pudo elegir efectivo.')}finally{setBusy(false)}}
 return <aside style={{position:'fixed',right:16,bottom:96,zIndex:35,width:'min(340px,calc(100vw - 32px))',background:'#fff',border:'1px solid rgba(23,33,27,.12)',borderRadius:18,padding:16,boxShadow:'0 14px 40px rgba(23,33,27,.14)'}} aria-label="Pago en efectivo"><small style={{fontWeight:900,letterSpacing:'.08em',color:'#128c4a'}}>FORMA DE PAGO</small><h3 style={{margin:'6px 0'}}>💵 Efectivo</h3>{selected?<p style={{margin:'0 0 8px',fontSize:14}}>Seleccionado. Este método <b>no tiene custodia electrónica de UGO</b>. Después de aprobar el trabajo, vos confirmás en UGO que entregaste el efectivo.</p>:<><p style={{margin:'0 0 10px',fontSize:14}}>Podés pagar directamente al profesional al finalizar. UGO registrará el pago cuando confirmes que entregaste el efectivo al profesional.</p><button type="button" onClick={chooseCash} disabled={busy} style={{width:'100%',minHeight:44,border:0,borderRadius:12,background:'#128c4a',color:'#fff',fontWeight:800}}>{busy?'Guardando…':'Elegir pago en efectivo'}</button></>}{message&&<small style={{display:'block',marginTop:8}}>{message}</small>}</aside>
}

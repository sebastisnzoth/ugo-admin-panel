...
export function ServiceHistoryPanel({role,embedded=false,openRequest=false}:Props){
 ...
 const[userId,setUserId]=useState<string|null>(null),[open,setOpen]=useState(embedded),[rows,setRows]=useState<Row[]>([]),[loading,setLoading]=useState(false),[error,setError]=useState(''),[filter,setFilter]=useState<'todos'|'completado'|'activo'>('todos')
 ...
 const panel=<section className={`ugo-history-panel${embedded?' embedded':''}`} onClick={e=>e.stopPropagation()}>
   <header><div><small>{role==='admin'?'CONTROL UGO':role==='provider'?'PROVEEDOR':'CLIENTE'}</small><h2>{title}</h2><p>{role==='admin'?'Todos los pedidos y trabajos de UGO.':role==='provider'?'Trabajos aceptados y realizados por vos.':'Pedidos activos y servicios ya realizados.'}</p></div>{!embedded&&<button type="button" onClick={()=>setOpen(false)} aria-label="Cerrar">×</button>}</header>

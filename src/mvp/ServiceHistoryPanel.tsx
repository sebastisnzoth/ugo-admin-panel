...
type Row={id:string;numero?:number|null;estado:string;descripcion?:string|null;tarifa?:number|string|null;created_at?:string|null;updated_at?:string|null;cliente_id?:string|null;proveedor_id?:string|null;categoria?:{nombre?:string|null;emoji?:string|null}|null;cliente?:Person|null;proveedor?:Person|null}

type Props={role:Role;embedded?:boolean;openRequest?:boolean}
...
export function ServiceHistoryPanel({role,embedded=false,openRequest=false}:Props){
 const sb=useMemo(()=>role==='admin'?adminSupabase:getRoleSupabase(role),[role])
 const[userId,setUserId]=useState<string|null>(null),[open,setOpen]=useState(embedded),[rows,setRows]=useState<Row[]>([]),[loading,setLoading]=useState(false),[error,setError]=useState(''),[filter,setFilter]=useState<'todos'|'completado'|'activo'|'cancelado'>('todos'),[cancellingId,setCancellingId]=useState(''),[actionNotice,setActionNotice]=useState('')
 ...
 const visible=rows.filter(r=>filter==='todos'?true:filter==='completado'?r.estado==='completado':filter==='cancelado'?r.estado==='cancelado':activeStates.has(r.estado))
 ...
 const panel=<section className={`ugo-history-panel${embedded?' embedded':''}${role==='client'?' ugo-client-history':''}${role==='provider'?' ugo-provider-history':''}`} onClick={e=>e.stopPropagation()}>
   <header><div><small>{role==='admin'?'CONTROL UGO':role==='provider'?'PROVEEDOR':'CLIENTE'}</small><h2>{title}</h2><p>{role==='admin'?'Todos los pedidos y trabajos de UGO.':role==='provider'?'Trabajos aceptados y realizados por vos.':'Acá está todo: pedidos activos, cancelados y servicios terminados.'}</p></div>{role==='client'&&<span className="ugo-client-history-total">{rows.length}</span>}{!embedded&&<button type="button" onClick={()=>setOpen(false)} aria-label="Cerrar">×</button>}</header>
   <div className="ugo-history-filters"><button type="button" className={filter==='todos'?'active':''} onClick={()=>setFilter('todos')}>Todos <b>{rows.length}</b></button><button type="button" className={filter==='activo'?'active':''} onClick={()=>setFilter('activo')}>Activos <b>{rows.filter(r=>activeStates.has(r.estado)).length}</b></button><button type="button" className={filter==='completado'?'active':''} onClick={()=>setFilter('completado')}>Completados <b>{rows.filter(r=>r.estado==='completado').length}</b></button>{role==='client'&&<button type="button" className={filter==='cancelado'?'active':''} onClick={()=>setFilter('cancelado')}>Cancelados <b>{rows.filter(r=>r.estado==='cancelado').length}</b></button>}<button type="button" className={role==='client'?'ugo-history-refresh':undefined} onClick={()=>void load()} disabled={loading} aria-label="Actualizar actividad">↻</button></div>
   ...
   <div className="ugo-history-list">{loading&&<div className="ugo-history-empty">Cargando actividad…</div>}{error&&<div className="ugo-history-error">{error}</div>}{!loading&&!error&&visible.length===0&&<div className="ugo-history-empty">Todavía no hay movimientos en esta sección.</div>}{!loading&&!error&&visible.map(r=><article key={r.id} className={role==='client'?`ugo-history-item state-${r.estado}`:undefined}>
    ...
    {role==='client'&&CLIENT_CANCELLABLE_STATES.has(r.estado)&&<div className="ugo-history-row-actions"><button type="button" className="ugo-history-cancel-button" disabled={Boolean(cancellingId)} onClick={()=>void cancelClientService(r.id)}>{cancellingId===r.id?'Cancelando…':'Cancelar solicitud'}</button></div>}
   </article>)}</div>
  </section>

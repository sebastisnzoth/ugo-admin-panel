...
export function ServiceHistoryPanel({role,embedded=false,openRequest=false}:Props){
 ...
 const[userId,setUserId]=useState<string|null>(null),[open,setOpen]=useState(embedded),[rows,setRows]=useState<Row[]>([]),[loading,setLoading]=useState(false),[error,setError]=useState(''),[filter,setFilter]=useState<'todos'|'completado'|'activo'>('todos'),[cancellingId,setCancellingId]=useState(''),[actionNotice,setActionNotice]=useState('')

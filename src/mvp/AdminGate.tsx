...
function installLegacyAdminBridge(validSession:any){
 ...
 auth.onAuthStateChange=(callback:any)=>originalOnAuthStateChange((event:any,session:any)=>{
  if(session){callback(event,{...session,user:{...session.user,email:'sebastianzoth@gmail.com'}});return}
...
export function AdminGate(){
 ...
 async function authorizeSession(session:any){if(!session)return false;const{profile,error}=await getAdminProfile(session.user.id);if(error||!profile||!profile.activo||!['admin','superadmin'].includes(String(profile.tipo)))return false;installLegacyAdminBridge(session);return true}
 ...
 if(allowed)return <><AdminPanel/><PixReconciliationPanel/><WhatsAppAdminInbox/></>

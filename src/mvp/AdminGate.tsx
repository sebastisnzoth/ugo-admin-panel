...
function resolveAdminLogin(value:string){
 ...
 return clean.includes('@')?clean.toLowerCase():`${clean.toLowerCase()}@ugo.local`
}

export function AdminGate({children}:AdminGateProps={}){
 const[checking,setChecking]=useState(true),[allowed,setAllowed]=useState(false),[identifier,setIdentifier]=useState(''),[password,setPassword]=useState(''),[newPassword,setNewPassword]=useState(''),[recovery,setRecovery]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState(''),[busy,setBusy]=useState(false)
 ...
 async function verify(){
  ...
  if(code){const{error:exchangeError}=await supabase.auth.exchangeCodeForSession(code);if(exchangeError){setError(exchangeError.message)}else{setRecovery(true);setNotice('Enlace validado. Definí una nueva contraseña para tu cuenta Admin.')}const clean=new URL(window.location.href);clean.searchParams.delete('code');window.history.replaceState({},'',clean.toString());setChecking(false);return}
 ...
 if(allowed)return children?<>{children}</>:<AdminPhase2/>
 if(recovery)return <div className="mvp-auth-page"><div className="mvp-auth-card"><div className="mvp-kicker">U.G.O. · ADMIN</div><h1>Nueva contraseña</h1><p>Definí una contraseña nueva para tu cuenta de administrador.</p><form onSubmit={saveNewPassword}><label>Nueva contraseña<input type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)} autoComplete="new-password" minLength={8} required/></label>{notice&&<div className="mvp-notice ok" role="status" aria-live="polite">{notice}</div>}{error&&<div className="mvp-form-error" role="alert">{error}</div>}<button type="submit" className="mvp-primary" disabled={busy}>{busy?'Guardando…':'Guardar contraseña →'}</button></form></div></div>
 return <div className="mvp-auth-page"><div className="mvp-auth-card"><div className="mvp-kicker">U.GO · ADMIN</div><h1>{appName==='development'?'Desarrollo UGO':'Panel de control'}</h1><p>Solo administradores autorizados.</p><form onSubmit={login}><label>Usuario o email<input type="text" value={identifier} onChange={e=>setIdentifier(e.target.value)} autoComplete="username" inputMode="email" placeholder="Adminugo" required/></label><label>Contraseña<input type="password" value={password} onChange={e=>setPassword(e.target.value)} autoComplete="current-password" required/></label>{notice&&<div className="mvp-notice ok" role="status" aria-live="polite">{notice}</div>}{error&&<div className="mvp-form-error" role="alert">{error}</div>}<button type="submit" className="mvp-primary" disabled={busy}>{busy?'Validando…':'Ingresar →'}</button><button type="button" className="mvp-secondary" onClick={sendRecovery} disabled={busy}>Olvidé mi contraseña</button></form></div></div>
}
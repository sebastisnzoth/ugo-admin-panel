...
export function AdminReportsCenter(){
 ...
 const load=useCallback(async()=>{
  ...
  const results=await Promise.all([
   supabase.from('servicios').select('id,numero,estado,tarifa,comision_ugo,ganancia_proveedor,categoria_id,proveedor_id,cliente_id,created_at,iniciado_at,completado_at,cancelado_at,categoria:categorias!servicios_categoria_id_fkey(nombre,emoji),proveedor:usuarios!servicios_proveedor_id_fkey(nombre,apellido)').gte('created_at',s).lte('created_at',e).order('created_at',{ascending:false}).limit(1000),
 ...
 const commercialServices=useMemo(()=>data.services.filter(s=>Number(s.tarifa||0)>1),[data.services])
 const stats=useMemo(()=>{
  ...
  const completedCommercial=commercialServices.filter(s=>s.estado==='completado')
  const volume=completedCommercial.reduce((a,s)=>a+Number(s.tarifa||0),0)
  const recordedCommission=completedCommercial.reduce((a,s)=>a+Number(s.comision_ugo||0),0)
  const recordedCommissionCount=completedCommercial.filter(s=>Number(s.comision_ugo||0)>0).length
  const estimatedCommission=volume*COMMISSION_RATE
  const gross=data.payments.reduce((a,p)=>a+Number(p.monto_bruto||0),0),paymentCommission=data.payments.reduce((a,p)=>a+Number(p.comision_ugo||0),0),providerNet=data.payments.reduce((a,p)=>a+Number(p.ganancia_proveedor||0),0)
  const clientsUnique=new Set(data.services.map(s=>s.cliente_id).filter(Boolean)).size,providersUnique=new Set(data.services.map(s=>s.proveedor_id).filter(Boolean)).size
  ...
  return{completed,canceled,active,volume,recordedCommission,recordedCommissionCount,estimatedCommission,gross,paymentCommission,providerNet,clientsUnique,providersUnique,clients,newProviders,verified,online,avgRating,disputes:data.disputes.length,total:data.services.length,testExcluded:data.services.length-commercialServices.length}
 },[data,commercialServices])

 const topCategories=useMemo(()=>Object.values(data.services.reduce((m:any,s:any)=>{const k=s.categoria_id||'sin';if(!m[k])m[k]={name:s.categoria?.nombre||'Sin categoría',emoji:s.categoria?.emoji||'•',count:0,completed:0,canceled:0};m[k].count++;if(s.estado==='completado')m[k].completed++;if(s.estado==='cancelado')m[k].canceled++;return m},{})).sort((a:any,b:any)=>b.count-a.count).slice(0,6) as any[],[data.services])
 ...
 const daily=useMemo(()=>Object.values(data.services.reduce((m:any,s:any)=>{const k=String(s.created_at||'').slice(0,10)||'sin-fecha';if(!m[k])m[k]={date:k,total:0,completed:0,canceled:0};m[k].total++;if(s.estado==='completado')m[k].completed++;if(s.estado==='cancelado')m[k].canceled++;return m},{})).sort((a:any,b:any)=>a.date.localeCompare(b.date)) as any[],[data.services])
 ...
 return <div className="ugo-reports">
  <section className="ugo-reports-hero"><div><small>CENTRO DE REPORTES</small><h3>Rendimiento de UGO</h3><p>Indicadores comerciales y operativos sobre datos reales del período seleccionado.</p></div><div><button onClick={()=>void load()} disabled={loading}>{loading?'Actualizando…':'↻ Actualizar'}</button><button className="primary" onClick={exportCurrent} disabled={loading}>Exportar CSV</button></div></section>
  ...
  <section className="ugo-reports-kpis executive"><article><small>PEDIDOS</small><strong>{stats.total}</strong><span>{stats.canceled} cancelados</span></article><article><small>COMPLETADOS</small><strong>{stats.completed}</strong><span>{pct(stats.completed,stats.completed+stats.canceled)} finalización</span></article><article><small>CONVERSIÓN</small><strong>{pct(stats.completed,stats.completed+stats.canceled)}</strong><span>{pct(stats.canceled,stats.completed+stats.canceled)} cancelación</span></article><article><small>VOLUMEN</small><strong>{money(stats.volume)}</strong><span>Servicios completados válidos</span></article><article><small>COMISIÓN UGO</small><strong>{money(stats.estimatedCommission)}</strong><span>Estimada al 15% · {stats.recordedCommissionCount} registradas</span></article><article><small>CLIENTES</small><strong>{stats.clientsUnique}</strong><span>{stats.providersUnique} proveedores en operaciones</span></article></section>
  {stats.testExcluded>0&&<div className="ugo-reports-note">Se excluyeron {stats.testExcluded} operaciones de prueba con tarifa ≤ R$ 1 de las métricas comerciales. Los conteos operativos las conservan para trazabilidad.</div>}
  ...
  {loading?<div className="ugo-reports-state">Cargando reporte…</div>:<>
   {view==='operations'&&<div className="ugo-reports-grid">
    <section className="wide"><header><div><small>PEDIDOS POR DÍA</small><h4>Actividad del período</h4></div></header><div className="ugo-reports-ranking">{daily.map((x:any)=><div key={x.date}><span>{new Date(`${x.date}T12:00:00`).toLocaleDateString('es-AR')}</span><b>{x.total}</b><small>{x.completed} completados · {x.canceled} cancelados</small></div>)}{!daily.length&&<p>Sin actividad.</p>}</div></section>
    <section><header><div><small>CATEGORÍAS</small><h4>Más solicitadas</h4></div></header><div className="ugo-reports-ranking">{topCategories.map((x:any,i)=><div key={i}><span>{x.emoji} {x.name}</span><b>{x.count}</b><small>{x.completed} completados · {x.canceled} cancelados</small></div>)}{!topCategories.length&&<p>Sin servicios en este período.</p>}</div></section>
    <section><header><div><small>COMPLETADOS / CANCELADOS</small><h4>Resultado operativo</h4></div></header><div className="ugo-reports-metrics"><div><span>Completados</span><b>{stats.completed}</b></div><div><span>Cancelados</span><b>{stats.canceled}</b></div><div><span>Finalización</span><b>{pct(stats.completed,stats.completed+stats.canceled)}</b></div><div><span>Cancelación</span><b>{pct(stats.canceled,stats.completed+stats.canceled)}</b></div></div></section>
    <section><header><div><small>PROVEEDORES</small><h4>Actividad del período</h4></div></header><div className="ugo-reports-ranking providers">{topProviders.map((x:any,i)=><div key={i}><span>{x.name}</span><b>{x.completed}</b><small>{x.count} asignados</small></div>)}{!topProviders.length&&<p>Sin proveedores asignados.</p>}</div></section>
    <section><header><div><small>INGRESOS</small><h4>Volumen comercial</h4></div></header><div className="ugo-reports-metrics"><div><span>Volumen completado</span><b>{money(stats.volume)}</b></div><div><span>Comisión estimada UGO</span><b>{money(stats.estimatedCommission)}</b></div><div><span>Comisión registrada</span><b>{money(stats.recordedCommission)}</b></div><div><span>Regla aplicada</span><b>15%</b></div></div></section>
   </div>}
   {view==='finance'&&<div className="ugo-reports-grid"><section><header><div><small>FINANZAS</small><h4>Movimiento del período</h4></div></header><div className="ugo-reports-metrics"><div><span>Pagos brutos</span><b>{money(stats.gross)}</b></div><div><span>Comisión registrada en pagos</span><b>{money(stats.paymentCommission)}</b></div><div><span>Neto proveedores</span><b>{money(stats.providerNet)}</b></div><div><span>Ticket medio</span><b>{money(data.payments.length?stats.gross/data.payments.length:0)}</b></div></div></section><section><header><div><small>PAGOS</small><h4>Estado</h4></div></header><div className="ugo-reports-ranking">{Object.entries(data.payments.reduce((m:any,p:any)=>{m[p.estado]=(m[p.estado]||0)+1;return m},{})).map(([k,v]:any)=><div key={k}><span>{String(k).replace(/_/g,' ')}</span><b>{v}</b></div>)}{!data.payments.length&&<p>Sin pagos en el período.</p>}</div></section></div>}

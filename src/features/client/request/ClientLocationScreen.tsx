 ...
 const useLocation=()=>{
  ...
  navigator.geolocation.getCurrentPosition(async pos=>{
   try{
    ...
    if(value){setSelectedPlace('');setAddress(value);setComplement('');setZone(nextZone);save(value,nextZone,null,'Ubicación actual','');savePickup(pos.coords.latitude,pos.coords.longitude,'current');try{sessionStorage.setItem('ugo:last-client-location',JSON.stringify({latitude:pos.coords.latitude,longitude:pos.coords.longitude,label:value,at:Date.now()}));if(session)await (supabase as any).from('perfiles_cliente').update({ubicacion:`POINT(${pos.coords.longitude} ${pos.coords.latitude})`}).eq('usuario_id',session.user.id)}catch(error){console.warn('UGO client GPS persistence unavailable',error)}void loadQuote(value,nextZone)}else setMessage('No pudimos obtener la dirección. Escribila abajo.')

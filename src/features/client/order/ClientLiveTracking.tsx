...
export function ClientLiveTracking({serviceId=null,embedded=false}:{serviceId?:string|null;embedded?:boolean}={}){
 ...
 return <aside className={rootClass} aria-live="polite"><header><div><small>SERVICIO #{service.numero}</small><strong>Profesional en camino</strong><p>Seguimiento en vivo hasta tu dirección.</p></div><span className="ugo-live-dot">●</span></header><Suspense fallback={<div className="ugo-active-map-wrap"><div className="ugo-active-eta is-syncing"><small>Cargando mapa en vivo…</small></div></div>}><ClientActiveMap supabase={supabase} serviceId={service.id} phase="en_camino"/></Suspense></aside>
}

...
type MatchingDeadlineRow={servicio_id:string;expira_at:string|null}
...
export function ClientHomeScreen({onOpenService}:Props){
 ...
 const[categories,setCategories]=useState<Category[]>([]),[orders,setOrders]=useState<ActiveOrder[]>([]),[matchingDeadlines,setMatchingDeadlines]=useState<Record<string,string>>({}),[query,setQuery]=useState(''),[showAll,setShowAll]=useState(false)
 ...
 const loadOrders=useCallback(async()=>{if(!session){setOrders([]);setMatchingDeadlines({});return}const{data}=await supabase.from('servicios').select('id,numero,estado,created_at,proveedor_id,proveedor:usuarios!servicios_proveedor_id_fkey(nombre),categoria:categorias(nombre,emoji)').eq('cliente_id',session.user.id).in('estado',ACTIVE_STATES).order('created_at',{ascending:false}).limit(3);const next=(data||[])as unknown as ActiveOrder[];setOrders(next);const ids=next.filter(order=>!order.proveedor_id&&['buscando','ofrecido'].includes(order.estado)).map(order=>order.id);if(!ids.length){setMatchingDeadlines({});return}const{data:offerRows}=await supabase.from('ofertas_servicio').select('servicio_id,expira_at').in('servicio_id',ids).eq('estado','pendiente').gt('expira_at',new Date().toISOString());const deadlines:Record<string,string>={};for(const row of(offerRows||[])as MatchingDeadlineRow[]){if(row.expira_at&&(!deadlines[row.servicio_id]||row.expira_at>deadlines[row.servicio_id]))deadlines[row.servicio_id]=row.expira_at}setMatchingDeadlines(deadlines)},[session,supabase])
 ...
 useEffect(()=>{
  ...
  sync()
  const ch=supabase.channel(`client-home-orders-${session.user.id}-${channelEpoch}`).on('postgres_changes',{event:'*',schema:'public',table:'servicios',filter:`cliente_id=eq.${session.user.id}`},sync).on('postgres_changes',{event:'*',schema:'public',table:'ofertas_servicio'},sync).subscribe(status=>{if(status==='SUBSCRIBED'){sync();return}if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'||status==='CLOSED'){sync();reconnect()}})
 ...
 return <main className="ugo-home-screen" aria-label="Inicio UGO Cliente">
  <section className="ugo-home-layout">
   <div className="ugo-home-hero">
    ...
    {orders.length>0&&<div className="ugo-home-active-orders" aria-label="Pedidos activos">{orders.map(order=><button type="button" key={order.id} className={order.proveedor_id?'is-assigned':'is-searching'} onClick={()=>openOrder(order.id)}><span className="ugo-home-order-icon">{order.proveedor_id?'✓':'⌁'}</span><span className="ugo-home-order-copy"><small>PEDIDO #{order.numero??String(order.id).slice(0,8)}</small><b>{order.proveedor_id?'Profesional asignado':(['buscando','ofrecido'].includes(order.estado)&&order.created_at?matchingDeadlines[order.id]?`Buscando profesional · ${formatCountdown(Math.max(0,new Date(matchingDeadlines[order.id]).getTime()-now))}`:'Buscando profesional':(STATE_LABEL[order.estado]||'Pedido activo'))}</b><strong>{order.proveedor_id?(order.proveedor?.nombre||'Profesional UGO'):(order.categoria?.nombre||'UGO está buscando')}</strong><em>{order.categoria?.emoji||'🧰'} {order.categoria?.nombre||'Servicio'} · Abrir pedido →</em></span></button>)}</div>}

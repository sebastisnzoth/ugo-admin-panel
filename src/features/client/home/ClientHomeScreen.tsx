...
type ActiveOrder={id:string;numero:number|null;estado:string;created_at:string|null;matching_expires_at:string|null;proveedor_id:string|null;proveedor:{nombre?:string|null}|null;categoria:{nombre?:string|null;emoji?:string|null}|null}
 ...
 return <main className="ugo-home-screen" aria-label="Inicio UGO Cliente">
  <section className="ugo-home-layout">
   <div className="ugo-home-hero">
    ...
    {orders.length>0&&<div className="ugo-home-active-orders" aria-label="Pedidos activos">{orders.map(order=><button type="button" key={order.id} className={order.proveedor_id?'is-assigned':'is-searching'} onClick={()=>openOrder(order.id)}><span className="ugo-home-order-icon">{order.proveedor_id?'✓':'⌁'}</span><span className="ugo-home-order-copy"><small>PEDIDO #{order.numero??String(order.id).slice(0,8)}</small><b>{order.proveedor_id?'Profesional asignado':(['buscando','ofrecido'].includes(order.estado)&&order.created_at?order.matching_expires_at?`Buscando profesional · ${formatCountdown(Math.max(0,new Date(order.matching_expires_at).getTime()-now))}`:'Buscando profesional':(STATE_LABEL[order.estado]||'Pedido activo'))}</b><strong>{order.proveedor_id?(order.proveedor?.nombre||'Profesional UGO'):(order.categoria?.nombre||'UGO está buscando')}</strong><em>{order.categoria?.emoji||'🧰'} {order.categoria?.nombre||'Servicio'} · Abrir pedido →</em></span></button>)}</div>}

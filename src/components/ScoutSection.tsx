...
export function SecScout(){
 ...
 const[gmailStatus,setGmailStatus]=useState<GmailStatus>({configured:false,connected:false,email:null,updatedAt:null}),[gmailBusy,setGmailBusy]=useState(false)
 ...
 const loadGmailStatus=useCallback(async()=>{
  const session=await supabase.auth.getSession();const token=session.data.session?.access_token;if(!token)return
  const response=await fetch('/api/scout/gmail',{headers:{Authorization:`Bearer ${token}`}}),payload=await response.json().catch(()=>({}))
  if(response.ok)setGmailStatus({configured:Boolean(payload.configured),connected:Boolean(payload.connected),email:payload.email||null,updatedAt:payload.updatedAt||null})
 },[])

 useEffect(()=>{void Promise.all([loadProspects(),loadDbProviders(),loadDemand(),loadGmailStatus()]).catch(e=>setError(e instanceof Error?e.message:'No se pudo cargar Scout.'))},[loadProspects,loadDbProviders,loadDemand,loadGmailStatus])
 useEffect(()=>{const params=new URLSearchParams(window.location.search),oauth=params.get('scout_gmail');if(oauth==='connected')setStatus('Gmail conectado. Ya podés enviar invitaciones desde Scout.');if(oauth==='error')setError('Google no pudo completar la conexión de Gmail. Revisá las credenciales OAuth e intentá nuevamente.');if(oauth){params.delete('scout_gmail');const q=params.toString();window.history.replaceState({},'',`${window.location.pathname}${q?`?${q}`:''}${window.location.hash}`)}},[])
 ...
 async function gmailAction(body:Record<string,unknown>){
  const request=async(token:string)=>fetch('/api/scout/gmail',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify(body)})
  let response=await request(await authToken());if(response.status===401)response=await request(await authToken(true))
  const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||`Gmail Scout respondió HTTP ${response.status}`);return payload
 }
 async function connectGmail(){
  if(gmailBusy)return;setGmailBusy(true);setError('')
  try{const payload=await gmailAction({action:'start'});if(!payload.url)throw new Error('Google no devolvió la URL de autorización.');window.location.assign(String(payload.url))}
 ...
 async function sendSelectedEmailCampaign(){
  ...
  try{
   ...
   const{data:saved,error:savedError}=await(supabase as any).from('prospectos_scouts').select('id,external_id').in('external_id',selectedEmails.map(p=>p.id))
   if(savedError)throw savedError
   const ids=(saved||[]).map((row:any)=>String(row.id))
   for(const batch of chunks(ids,20)){
    setCampaignProgress(`Gmail ${done}/${ids.length} · enviados ${sent} · errores ${failed}`)
    const payload=await gmailAction({action:'send_campaign',ids:batch,subject:emailSubject,message:recruitmentText,zona:locationLabel})
    sent+=Number(payload.sent||0);failed+=Number(payload.failed||0);done+=batch.length
   }
   await Promise.all([loadProspects(),loadGmailStatus()]);setCampaignProgress('');setStatus(`Campaña Gmail finalizada: ${sent} enviados · ${failed} errores.`)
  }catch(e){setError(e instanceof Error?e.message:'No se pudo completar la campaña Gmail.')}
  finally{setCampaignBusy(false)}
 }
 ...
 async function sendSavedEmailCampaign(){
  ...
  try{
   for(const batch of chunks(savedEmails,20)){
    ...
    const payload=await gmailAction({action:'send_campaign',ids:batch.map(p=>p.id),subject:emailSubject,message:recruitmentText,zona:locationLabel})
    sent+=Number(payload.sent||0);failed+=Number(payload.failed||0);done+=batch.length
   }
 ...
 async function contactProspectEmail(p:Prospect){
  const email=String(p.email||'').trim();if(!email){setError('Este prospecto todavía no tiene email.');return}
  if(p.no_contactar){setError('Este prospecto está marcado como no contactar.');return}
  if(!gmailStatus.connected){setError('Conectá una cuenta Gmail en Scout antes de enviar.');return}
  setBusyId('email-'+p.id);setError('')
  try{await gmailAction({action:'send',prospectId:p.id,subject:emailSubject,message:recruitmentText,zona:p.ciudad||locationLabel});await loadProspects();setStatus(`Invitación Gmail enviada a ${p.nombre} desde ${gmailStatus.email||'la cuenta conectada'}.`)}
  catch(e){setError(e instanceof Error?e.message:'No se pudo enviar el email.')}
  finally{setBusyId('')}
 }
 ...
 return <div className="ugo-scout">
  ...
  <section className="ugo-scout-recruit">
   <header><div><small>RECLUTAMIENTO SCOUT</small><strong>{selectedResults.length} seleccionados</strong></div><span>WhatsApp y Gmail envían hasta 20 por lote para evitar disparos accidentales.</span></header>
   <div className={`ugo-scout-gmail ${gmailStatus.connected?'connected':''}`}><div><small>GMAIL SCOUT</small><strong>{gmailStatus.connected?`✓ ${gmailStatus.email||'Cuenta conectada'}`:gmailStatus.configured?'Cuenta lista para conectar':'Falta configurar OAuth de Google'}</strong><span>{gmailStatus.connected?'Los envíos quedan registrados en la ficha y en el embudo.':'Conectá una cuenta Google una sola vez para enviar invitaciones desde el panel.'}</span></div><div className="ugo-scout-actions">{gmailStatus.connected?<button type="button" onClick={()=>void disconnectGmail()} disabled={gmailBusy}>{gmailBusy?'Procesando…':'Desconectar Gmail'}</button>:<button type="button" className="primary" onClick={()=>void connectGmail()} disabled={gmailBusy||!gmailStatus.configured}>{gmailBusy?'Abriendo Google…':'Conectar Gmail'}</button>}</div></div>
   ...
   <div className="ugo-scout-actions">
    ...
    <button type="button" onClick={openEmailDraft} disabled={!selectedEmails.length||campaignBusy}>Borrador ({selectedEmails.length})</button>
    <button type="button" className="primary" onClick={()=>void sendSelectedEmailCampaign()} disabled={!selectedEmails.length||campaignBusy||!gmailStatus.connected}>Gmail ({selectedEmails.length})</button>
   </div>

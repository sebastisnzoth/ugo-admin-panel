...
export function SecMapaOperativo(){
  ...
  return <div className="ugo-admin-map" style={{height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}}>
    <div className="ugo-admin-map-toolbar" style={{padding:'8px 14px',borderBottom:'1px solid #e5e7eb',display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',background:'#fff',color:'#101828'}}>
      <strong style={{fontSize:14,color:'#101828',fontWeight:850}}>🗺 Mapa Operativo</strong>{pill('Proveedores',prov.length,'#276EF1')}{pill('Online',online,'#05944F')}{pill('Offline',offline,'#F59E0B')}{pill('Clientes',clients,'#276EF1')}{pill('Servicios',services.length,'#8B5CF6')}
      <div style={{flex:1}}/><input value={geoSearch} onChange={e=>setGeoSearch(e.target.value)} onKeyDown={e=>e.key==='Enter'&&goLocation()} placeholder="Buscar ciudad, barrio..." style={{padding:'7px 10px',border:'1px solid #d0d5dd',borderRadius:8,width:190,background:'#fff',color:'#101828',fontWeight:650}}/><select value={geoRadius} onChange={e=>setGeoRadius(Number(e.target.value))} style={{padding:'7px',border:'1px solid #d0d5dd',borderRadius:8,background:'#fff',color:'#101828',fontWeight:700}}><option value={0}>Sin radio</option><option value={2000}>2 km</option><option value={5000}>5 km</option><option value={10000}>10 km</option><option value={20000}>20 km</option></select><button onClick={goLocation} disabled={geoBusy} style={{padding:'7px 10px',border:0,borderRadius:8,background:'#276EF1',color:'#fff'}}>🗺 Ir</button><button onClick={load} style={{padding:'7px 10px',border:0,borderRadius:8,background:'#111',color:'#fff'}}>↻</button><label style={{fontSize:11,color:'#475467',fontWeight:750,display:'inline-flex',alignItems:'center',gap:5}}><input type="checkbox" checked={auto} onChange={e=>setAuto(e.target.checked)} style={{width:15,height:15,accentColor:'#05944F'}}/> Auto</label><span style={{fontSize:10,color:'#667085',fontWeight:650}}>{visible.length} visibles · {lastUpd}</span>
    </div>
    ...
    <div className="ugo-admin-map-body" style={{flex:1,display:'flex',minHeight:0}}>
      <aside className="ugo-admin-map-filters" style={{width:210,background:'#fff',color:'#101828',borderRight:'1px solid #e5e7eb',padding:12,overflowY:'auto'}}>
        <small style={{fontWeight:900,color:'#475467',fontSize:11,letterSpacing:'.05em'}}>TIPO</small><label style={{display:'flex',alignItems:'center',gap:7,marginTop:9,fontSize:13,color:'#101828',fontWeight:750,lineHeight:1.35}}><input type="checkbox" checked={showProv} onChange={e=>setShowProv(e.target.checked)} style={{width:16,height:16,accentColor:'#05944F'}}/> 🔧 Proveedores</label><label style={{display:'flex',alignItems:'center',gap:7,marginTop:7,fontSize:13,color:'#101828',fontWeight:750,lineHeight:1.35}}><input type="checkbox" checked={showCli} onChange={e=>setShowCli(e.target.checked)} style={{width:16,height:16,accentColor:'#276EF1'}}/> 👤 Clientes</label>
        <div style={{marginTop:18}}><small style={{fontWeight:900,color:'#475467',fontSize:11,letterSpacing:'.05em'}}>ESTADO</small>{[['todos','⚫ Todos'],['online','🟢 Online'],['offline','🟡 Activos / Offline'],['inactivo','🔴 Inactivos']].map(([v,l])=><label key={v} style={{display:'flex',alignItems:'center',gap:7,marginTop:7,fontSize:12,color:'#344054',fontWeight:700,lineHeight:1.35}}><input type="radio" checked={status===v} onChange={()=>setStatus(v as any)} style={{width:15,height:15,accentColor:'#05944F'}}/> {l}</label>)}</div>
        <div style={{marginTop:18}}><small style={{fontWeight:900,color:'#475467',fontSize:11,letterSpacing:'.05em'}}>CATEGORÍA</small><div onClick={()=>setCategory('todos')} style={{marginTop:8,padding:'7px 8px',cursor:'pointer',borderRadius:8,background:category==='todos'?'#e8f5ee':'transparent',color:'#101828',fontSize:12,fontWeight:750,border:category==='todos'?'1px solid #b7e1cb':'1px solid transparent'}}>📍 Todos</div>{cats.map(c=><div key={c.slug} onClick={()=>setCategory(c.slug)} style={{padding:'7px 8px',cursor:'pointer',borderRadius:8,background:category===c.slug?'#e8f5ee':'transparent',color:'#344054',fontSize:12,fontWeight:700,border:category===c.slug?'1px solid #b7e1cb':'1px solid transparent'}}>{c.emoji||'🔧'} {c.nombre}</div>)}</div>
        <div style={{marginTop:18}}><small style={{fontWeight:900,color:'#475467',fontSize:11,letterSpacing:'.05em'}}>ZONA / BARRIO</small><input value={zone} onChange={e=>setZone(e.target.value)} placeholder="Filtrar..." style={{marginTop:8,width:'100%',boxSizing:'border-box',padding:'8px 9px',border:'1px solid #d0d5dd',borderRadius:8,background:'#fff',color:'#101828',fontSize:12,fontWeight:650}}/></div>
      </aside>
      <div className="ugo-admin-map-stage" style={{flex:1,position:'relative'}}><div ref={mapEl} className="ugo-admin-map-canvas" style={{width:'100%',height:'100%'}}/>{loading&&<div style={{position:'absolute',top:12,left:'50%',transform:'translateX(-50%)',background:'#fff',color:'#101828',padding:'8px 14px',borderRadius:12,boxShadow:'0 2px 10px #0002',zIndex:999,fontSize:12,fontWeight:700}}>⏳ Cargando mapa...</div>}{!loading&&!error&&visible.length===0&&<div style={{position:'absolute',top:12,left:'50%',transform:'translateX(-50%)',background:'#fff',color:'#101828',padding:'9px 15px',borderRadius:12,boxShadow:'0 2px 10px #0002',zIndex:999,fontSize:12,fontWeight:700}}>📍 No hay ubicaciones para estos filtros</div>}</div>
      {selected&&<aside className="ugo-admin-map-detail" style={{width:240,borderLeft:'1px solid #e5e7eb',background:'#fff',color:'#101828',padding:14,overflowY:'auto'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}><strong style={{color:'#101828',fontSize:13}}>{selected.tipo==='proveedor'?'🔧 Proveedor':'👤 Cliente'}</strong><button onClick={()=>setSelected(null)} aria-label="Cerrar detalle" style={{border:0,background:'none',color:'#344054',fontSize:18,cursor:'pointer'}}>✕</button></div><h3 style={{color:'#101828',margin:'12px 0 8px'}}>{selected.nombre} {selected.apellido||''}</h3><p style={{fontSize:12,color:'#475467',fontWeight:650}}>{selected.tipo==='proveedor'?(selected.categoria||'Sin categoría'):'Cliente'}</p><p style={{fontSize:12,color:'#475467'}}>📍 {selected.zona||selected.endereco||'Sin zona'}</p><p style={{fontSize:12,color:'#475467'}}>GPS: {selected.lat?.toFixed(5)}, {selected.lng?.toFixed(5)}</p>{selected.telefono&&<a href={`https://wa.me/${selected.telefono.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" style={{color:'#175CD3',fontWeight:800}}>WhatsApp</a>}</aside>}
    </div>
  </div>
}

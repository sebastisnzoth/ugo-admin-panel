...
.hugo-free-orb{width:96px;height:96px;cursor:pointer;border-radius:50%;background:radial-gradient(circle at 34% 27%,#12f58c,#05944F 46%,#024d2c 72%,#012d1a);box-shadow:0 12px 38px rgba(5,148,79,.22);animation:hugoFloat 3s ease-in-out infinite}
.hugo-free-status{height:20px;font-size:12px;color:#777}.hugo-free-status.active{color:#05944F}
...
function adminServiceLookup(value:unknown){
 const key=String(value||'').trim()
 if(!key)return null
 if(UUID_RE.test(key))return{field:'id' as const,value:key}
 if(/^\d+$/.test(key)){const numero=Number(key);if(Number.isSafeInteger(numero)&&numero>0)return{field:'numero' as const,value:numero}}
 return null
}
function adminUserSearchTerm(value:unknown){return String(value||'').trim().replace(/[%_]/g,'').slice(0,120)}
function uniqueAdminUsers(groups:any[][]){const seen=new Set<string>(),rows:any[]=[];for(const group of groups)for(const row of group||[]){const id=String(row?.id||'');if(!id||seen.has(id))continue;seen.add(id);rows.push(row);if(rows.length>=10)return rows}return rows}
...
export function ConversationalOrb({metrics,role='admin',section='dashboard',extraContext,onVoiceActiveChange}:{metrics?:any;role?:HugoRole;section?:string;extraContext?:any;onVoiceActiveChange?:(active:boolean)=>void}){
 ...
 const endRef=useRef<HTMLDivElement>(null),voiceSessionRef=useRef(false);
 useEffect(()=>{onVoiceActiveChange?.(open)},[open,onVoiceActiveChange]);
 useEffect(()=>{endRef.current?.scrollIntoView({behavior:'smooth'})},[msgs]);
 const stopAudio=useCallback(()=>{try{window.UGOVoiceBridge?.stopSpeaking?.()}catch{}},[])
 useEffect(()=>{const bridge=window.UGOVoiceBridge;const onNativeResult=(event:Event)=>{const detail=(event as CustomEvent<{text?:string;final?:boolean}>).detail||{},text=String(detail.text||'').trim();if(detail.final===false){if(text)setOrbState('listening');return}if(text)setInput(text)};const onNativeState=(event:Event)=>{const detail=(event as CustomEvent<{state?:string}>).detail||{};if(!voiceSessionRef.current)return;if(['connecting','ready','hearing'].includes(String(detail.state||'')))setOrbState('listening')};const onNativeOutput=(event:Event)=>{const text=String((event as CustomEvent<{text?:string}>).detail?.text||'').trim();if(text)setMsgs(p=>[...p,{role:'hugo',text}])};const onNativeTool=async(event:Event)=>{const detail=(event as CustomEvent<{id?:string;name?:string;args?:Record<string,unknown>}>).detail||{},id=String(detail.id||''),name=String(detail.name||''),args=detail.args||{};if(!id||!name||!bridge?.sendToolResponse)return;try{let response:any={ok:false,code:'UNKNOWN_TOOL',message:'Herramienta Admin no disponible'};if(name==='admin_get_operational_summary')response={ok:true,data:JSON.parse(await buildLiveContext(metrics,role,section,extraContext))};else if(name==='admin_find_service'){const lookup=adminServiceLookup(args.service_id);if(!lookup)response={ok:false,code:'INVALID_SERVICE_KEY',message:'Indicá un UUID o número de servicio válido'};else{const query=(supabase as any).from('servicios').select('id,numero,estado,zona,tarifa,created_at,descripcion,cliente_id,proveedor_id,categoria_id'),{data,error}=await query.eq(lookup.field,lookup.value).limit(5);response=error?{ok:false,code:'QUERY_FAILED',message:error.message}:{ok:true,data:{services:data||[]}}}}else if(name==='admin_get_service_history'){const lookup=adminServiceLookup(args.service_id);if(!lookup)response={ok:false,code:'INVALID_SERVICE_KEY',message:'Indicá un UUID o número de servicio válido'};else{const query=(supabase as any).from('servicios').select('id,numero,estado'),{data:services,error:serviceError}=await query.eq(lookup.field,lookup.value).limit(2);if(serviceError)response={ok:false,code:'QUERY_FAILED',message:serviceError.message};else if(!services?.length)response={ok:false,code:'SERVICE_NOT_FOUND',message:'No encontré ese servicio'};else if(services.length>1)response={ok:false,code:'AMBIGUOUS_SERVICE',message:'Hay más de un servicio que coincide'};else{const target=services[0],{data:events,error:eventError}=await(supabase as any).from('servicio_estado_eventos').select('id,servicio_id,actor_role,estado_anterior,estado_nuevo,motivo,created_at').eq('servicio_id',target.id).order('created_at',{ascending:true}).limit(100);response=eventError?{ok:false,code:'QUERY_FAILED',message:eventError.message}:{ok:true,data:{service:target,events:events||[]}}}}}else if(name==='admin_list_disputes'){const{data,error}=await(supabase as any).from('disputas').select('id,numero,estado,monto_disputado,motivo,created_at,servicio_id').in('estado',['abierta','en_revision']).order('created_at',{ascending:false}).limit(40);response=error?{ok:false,code:'QUERY_FAILED',message:error.message}:{ok:true,data:{disputes:data||[]}}}else if(name==='admin_find_user'){const q=adminUserSearchTerm(args.query),select='id,nombre,apellido,email,tipo,activo,online,zona,pais';if(!q)response={ok:false,code:'INVALID_USER_QUERY',message:'Indicá un usuario, nombre o email'};else if(UUID_RE.test(q)){const{data,error}=await(supabase as any).from('usuarios').select(select).eq('id',q).limit(10);response=error?{ok:false,code:'QUERY_FAILED',message:error.message}:{ok:true,data:{users:data||[]}}}else{const token=q.split(/\s+/)[0]||q,[byName,byLastName,byEmail]=await Promise.all([(supabase as any).from('usuarios').select(select).ilike('nombre',`%${token}%`).limit(10),(supabase as any).from('usuarios').select(select).ilike('apellido',`%${token}%`).limit(10),(supabase as any).from('usuarios').select(select).ilike('email',`%${q}%`).limit(10)]),queryError=byName.error||byLastName.error||byEmail.error;response=queryError?{ok:false,code:'QUERY_FAILED',message:queryError.message}:{ok:true,data:{users:uniqueAdminUsers([byName.data||[],byLastName.data||[],byEmail.data||[]])}}}}bridge.sendToolResponse(id,name,response)}catch(error){bridge.sendToolResponse(id,name,{ok:false,code:'TOOL_FAILED',message:error instanceof Error?error.message:'La consulta falló'})}};const onNativeError=()=>{voiceSessionRef.current=false;setOrbState('idle')};window.addEventListener('ugo:native-voice-result',onNativeResult as EventListener);window.addEventListener('ugo:native-voice-state',onNativeState as EventListener);window.addEventListener('ugo:native-voice-output',onNativeOutput as EventListener);window.addEventListener('ugo:native-voice-tool-call',onNativeTool as EventListener);window.addEventListener('ugo:native-voice-error',onNativeError);return()=>{window.removeEventListener('ugo:native-voice-result',onNativeResult as EventListener);window.removeEventListener('ugo:native-voice-state',onNativeState as EventListener);window.removeEventListener('ugo:native-voice-output',onNativeOutput as EventListener);window.removeEventListener('ugo:native-voice-tool-call',onNativeTool as EventListener);window.removeEventListener('ugo:native-voice-error',onNativeError);if(voiceSessionRef.current)bridge?.stopListening?.();stopAudio()}},[extraContext,metrics,role,section,stopAudio]);
 ...
 const status=orbState==='listening'?'Te escucho. Hablame…':voiceSessionRef.current?'Conversación activa':'Tocá el micrófono para conversar';
 return <><style>{CSS}</style><button className="hugo-free-trigger" aria-label={`Abrir Hugo ${roleLabel}`} title={`Hugo ${roleLabel} · Gemini + datos en vivo`} onClick={openConversation}><span><b>Hugo</b><small className="hugo-free-trigger-role">{roleLabel}</small></span></button>{open&&<div className="hugo-free-overlay"><div className="hugo-free-head"><div><strong>U.G.O. · HUGO</strong><small>{roleLabel.toUpperCase()} · GEMINI LIVE · CONTROL CENTER</small></div><button className="hugo-free-close" onClick={closeOrb}>×</button></div><div className="hugo-free-orb-area"><div className="hugo-free-orb-wrap"><div className="hugo-free-ring hidden"/><button type="button" aria-label="Activar o pausar Hugo" className={`hugo-free-orb ${orbState}`} onClick={toggleMic}/></div><div className="hugo-free-status">{status}</div></div><div className="hugo-voice-caption">{msgs[msgs.length-1]?.text||input||'Te escucho…'}</div><div ref={endRef}/></div>}</>;
}

...
.hugo-free-overlay{position:fixed;inset:0;z-index:9500;background:rgba(247,248,249,.94);backdrop-filter:blur(12px);display:flex;flex-direction:column;align-items:center;justify-content:center;color:#111;font-family:Inter,system-ui,sans-serif;pointer-events:none}
.hugo-free-head{position:fixed;top:14px;left:50%;transform:translateX(-50%);display:flex;align-items:center;justify-content:center;pointer-events:auto}
...
.hugo-free-close{position:fixed;right:18px;top:15px;width:34px;height:34px;border:0;border-radius:50%;background:#eceeef;cursor:pointer;font-size:18px;pointer-events:auto}
...
export function ConversationalOrb({metrics,role='admin',section='dashboard',extraContext}:{metrics?:any;role?:HugoRole;section?:string;extraContext?:any}){
 ...
 const recognitionRef=useRef<any>(null),endRef=useRef<HTMLDivElement>(null),msgsRef=useRef<Msg[]>(msgs),audioRef=useRef<HTMLAudioElement|null>(null),audioUrlRef=useRef(''),voiceSessionRef=useRef(false),openRef=useRef(false),speakingTimerRef=useRef<number>(0);
 useEffect(()=>{openRef.current=open},[open]);
 useEffect(()=>{msgsRef.current=msgs;endRef.current?.scrollIntoView({behavior:'smooth'})},[msgs,loading]);
 const stopAudio=useCallback(()=>{if(speakingTimerRef.current){window.clearTimeout(speakingTimerRef.current);speakingTimerRef.current=0}try{audioRef.current?.pause()}catch{}audioRef.current=null;if(audioUrlRef.current){URL.revokeObjectURL(audioUrlRef.current);audioUrlRef.current=''}window.speechSynthesis?.cancel()},[]);
 ...
 useEffect(()=>{const bridge=window.UGOVoiceBridge,SR=window.SpeechRecognition||window.webkitSpeechRecognition;setVoiceAvailable(Boolean(bridge?.isAvailable?.()||SR));const onNativeResult=(event:Event)=>{const detail=(event as CustomEvent<{text?:string;final?:boolean}>).detail||{},text=String(detail.text||'').trim();if(detail.final===false){if(text)setOrbState('listening');return}if(text){bridge?.pauseListening?.();void handleSend(text)}};const onNativeState=(event:Event)=>{const detail=(event as CustomEvent<{state?:string}>).detail||{};if(!voiceSessionRef.current)return;if(['connecting','ready','hearing'].includes(String(detail.state||'')))setOrbState('listening')};const onNativeError=()=>{const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(voiceSessionRef.current&&SR&&recognitionRef.current){setOrbState('listening');try{recognitionRef.current.start()}catch{setOrbState('idle')}}else{voiceSessionRef.current=false;setOrbState('idle')}};window.addEventListener('ugo:native-voice-result',onNativeResult as EventListener);window.addEventListener('ugo:native-voice-state',onNativeState as EventListener);window.addEventListener('ugo:native-voice-error',onNativeError);if(SR){const rec=new SR();rec.lang='es-AR';rec.continuous=false;rec.interimResults=false;rec.onresult=(e:any)=>{const text=String(e?.results?.[0]?.[0]?.transcript||'').trim();if(text)void handleSend(text)};rec.onend=()=>{if(voiceSessionRef.current&&openRef.current&&orbState!=='speaking'&&!loading){window.setTimeout(()=>{try{rec.start();setOrbState('listening')}catch{}},180)}else setOrbState(v=>v==='listening'?'idle':v)};rec.onerror=()=>setOrbState('idle');recognitionRef.current=rec}return()=>{window.removeEventListener('ugo:native-voice-result',onNativeResult as EventListener);window.removeEventListener('ugo:native-voice-state',onNativeState as EventListener);window.removeEventListener('ugo:native-voice-error',onNativeError);try{recognitionRef.current?.abort?.()}catch{};if(voiceSessionRef.current)bridge?.stopListening?.();stopAudio()}},[handleSend,stopAudio]);
 const startConversation=useCallback(async()=>{stopAudio();voiceSessionRef.current=true;setOrbState('listening');const bridge=window.UGOVoiceBridge;if(bridge?.isAvailable?.()){try{await bridge.startListening();return}catch{}}if(recognitionRef.current){try{recognitionRef.current.start();return}catch{}}voiceSessionRef.current=false;setOrbState('idle')},[stopAudio]);

...
- Gate contractual de ownership de push recuperado el 12/09: se separa `SET` de `WHERE` y se agregan tres mutaciones que deben fallar (reasignación, filtro ausente, `OR true`). Validación focal: 6/6. `npm test`: 42 aprobados, 0 fallos y 1 caso RPC/RLS omitido. Es evidencia contractual local; no se declara RLS real, CI verde ni cierre P0.
- Push de `9f77d7c` confirmado a `origin/main`; fetch posterior verificó igualdad con `main` antes del bloque de ownership. Ese nuevo bloque es local; no se consultó producción ni Arena ni se verificó deploy/smoke.
- `npm run build` del bloque de ownership pasa con Vite 8.0.16 del lockfile (warning de tamaño de chunks). El lint crítico de las 31 superficies pasó en el bloque anterior (`9f77d7c`) con 0 errores y 2 warnings; no se repitió ese gate separado. Esta evidencia es local y no valida deploy.

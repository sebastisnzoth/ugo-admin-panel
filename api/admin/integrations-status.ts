...
export default async function handler(req:VercelRequest,res:VercelResponse){
 try{
  ...
  const scoutGmailConfigured=Boolean((process.env.SCOUT_GMAIL_CLIENT_ID||process.env.GOOGLE_GMAIL_CLIENT_ID||process.env.GOOGLE_CALENDAR_CLIENT_ID)&&(process.env.SCOUT_GMAIL_CLIENT_SECRET||process.env.GOOGLE_GMAIL_CLIENT_SECRET||process.env.GOOGLE_CALENDAR_CLIENT_SECRET))
  ...
  const integrations:Integration[]=[
   ...
   {id:'scout_gmail',label:'Gmail · Scout',category:'messaging',configured:scoutGmailConfigured,enabled:scoutGmailConfigured,environment:'server',runtimeSource:'SCOUT_GMAIL_CLIENT_ID + SCOUT_GMAIL_CLIENT_SECRET (fallback Google OAuth compartido)',note:'OAuth server-side con scope gmail.send. Scout envía invitaciones y audita el contacto sin exponer refresh tokens al navegador.'},

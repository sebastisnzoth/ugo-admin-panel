...
test('legacy app=web is visibly isolated as fictitious demo data',async()=>{const app=await read('src/mvp/MvpApp.tsx'),boundary=await read('src/mvp/UgoDemoBoundary.tsx').catch(()=> '');assert.match(app,/app==='web'.*<UgoDemoBoundary><UgoWeb\/><\/UgoDemoBoundary>/s);assert.match(boundary,/DEMO · DATOS FICTICIOS/);assert.match(boundary,/No representa la operación real de UGO/);assert.match(boundary,/role="status"/)})

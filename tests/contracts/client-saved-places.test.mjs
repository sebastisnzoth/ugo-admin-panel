...
test('location step offers saved places and current GPS',async()=>{const s=await read('src/features/client/request/ClientLocationScreen.tsx');assert.match(s,/Tus lugares/);assert.match(s,/choosePlace\(place\)/);assert.match(s,/direcciones_cliente/);assert.match(s,/Usar mi ubicación/);assert.match(s,/ugo:last-client-location/)})
...
test('home recenters from browser geolocation',async()=>{const h=await read('src/features/client/home/ClientHomeScreen.tsx');assert.match(h,/const locateUser=useCallback/);assert.match(h,/navigator\.geolocation\.getCurrentPosition/);assert.match(h,/ugo:last-client-location/);assert.match(h,/Mi ubicación/)})

...
test('profile manages places and starts an order there',async()=>{const p=await read('src/mvp/client/ClientProfilePanel.tsx');assert.match(p,/\+ Guardar lugar/);assert.match(p,/Pedir acá/);assert.match(p,/Hacer principal/);assert.match(p,/set_direccion_cliente_predeterminada/);assert.match(p,/flow\.navigate\('request'\)/)})

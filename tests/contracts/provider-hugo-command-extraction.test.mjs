...
test('provider Hugo executes declared operational tools through guarded provider actions',async()=>{
 ...
 for(const tool of ['provider_set_online','provider_set_offline','provider_list_opportunities','provider_accept_job','provider_reject_job','provider_get_current_job','provider_mark_en_route','provider_mark_arrived','provider_start_job','provider_complete_job'])assert.match(bridge,new RegExp(tool))
 ...
 assert.match(live,/provider_mark_arrived/)
 assert.match(live,/nunca propongas lat\/lng/)
 assert.doesNotMatch(live,/provider_update_service_status/)

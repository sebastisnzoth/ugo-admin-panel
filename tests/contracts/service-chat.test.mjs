...
test('service chat converges even when a realtime event is missed',async()=>{
 ...
 assert.match(component,/const onOnline=\(\)=>\{resync\(\);reconnect\(\)\}/)
 assert.match(component,/window\.addEventListener\('online',onOnline\)/)
 ...
 assert.match(component,/status==='CHANNEL_ERROR'\|\|status==='TIMED_OUT'/)
 assert.match(component,/resync\(\);reconnect\(\)/)
 assert.match(component,/window\.clearInterval\(fallback\)/)
 assert.match(component,/window\.removeEventListener\('online',onOnline\)/)

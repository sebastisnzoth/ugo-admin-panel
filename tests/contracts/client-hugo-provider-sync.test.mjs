...
test('canonical Hugo announces only real provider availability and recommendations',()=>{assert.match(voice,/loadVoiceAvailability/);assert.match(voice,/voiceAvailabilityText/);assert.match(catalog,/Ahora no veo profesionales de/);assert.match(catalog,/Encontré 1 profesional de/);assert.match(catalog,/Por los datos reales de UGO/);assert.match(catalog,/servicios_completados/);assert.match(catalog,/tarifa_base/)})

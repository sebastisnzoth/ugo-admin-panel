...
const bridge=await readFile(new URL('../../src/features/client/radar/ClientProviderRadarBridge.tsx',import.meta.url),'utf8')
...
test('canonical Hugo uses real provider availability through the Gemini Live search tool',()=>{assert.match(voice,/name==='search_providers'/);assert.match(voice,/loadVoiceAvailability\(current\.category\)/);assert.match(voice,/providers:result\.providers\.slice\(0,5\)/);assert.match(catalog,/Ahora no veo profesionales de/);assert.match(catalog,/Encontré 1 profesional de/);assert.match(catalog,/Por los datos reales de UGO/);assert.match(catalog,/servicios_completados/);assert.match(catalog,/tarifa_base/)})

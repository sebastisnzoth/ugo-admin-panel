...
test('Client and Provider require the persistent Gemini Live audio speaker and never mask failure with browser speech',()=>{
 ...
 assert.match(api,/requestedLiveMode=String\(req.body\?\.voice_live_mode\|\|'conversation'\)/)
 assert.match(api,/liveMode=requestedLiveMode==='transcribe'\?'transcribe':requestedLiveMode==='speaker'\?'speaker':'conversation'/)
...
test('Gemini Live declares bounded role tools in the same persistent session',()=>{
 ...
 assert.match(bridge,/rate_service/)
 assert.match(bridge,/required:\['service_id','confirmed'\]/)
 assert.match(bridge,/required:\['service_id','score','confirmed'\]/)

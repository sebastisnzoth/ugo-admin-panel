...
test('canonical Hugo voice requires Gemini Live and has no parallel browser recognition fallback',()=>{assert.match(voice,/UGOVoiceBridge/);assert.match(voice,/bridge.startListening\(\)/);assert.doesNotMatch(voice,/SpeechRecognition|webkitSpeechRecognition|rec\.onresult|speechSynthesis/);assert.match(api,/voice_live_token===true/)})

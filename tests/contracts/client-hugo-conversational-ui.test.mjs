...
test('home/request keep transactional checkout hidden while the Hugo orb stays available',()=>{assert.ok(root.includes("const canonical=flow.screen==='home'||flow.screen==='request'"));assert.ok(root.includes("flow.screen==='request'&&<ClientNeedScreen/>"));assert.ok(globals.includes('<ClientHugoBridge/>'));assert.doesNotMatch(surfaces,/ClientPaymentChoice|ClientLiveTracking|ClientCompletionReview|ServiceChat/);assert.match(voice,/ugo-hugo-voice-controller/);assert.doesNotMatch(voice,/ugo-hugo-stage-card/)})

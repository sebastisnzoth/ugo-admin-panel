...
test('client mounts one canonical Hugo companion instead of the legacy voice-order modal',()=>{assert.match(root,/ClientHugoBridge/);assert.match(root,/flow\.screen==='request'&&<ClientNeedScreen/);assert.match(root,/!canonical&&!detailOpen&&<ClientHugoBridge/);assert.doesNotMatch(root,/ClientHugoVoiceOrder/)})
test('canonical checkout and Hugo never overlap',()=>{assert.match(root,/canonical=flow\.screen==='home'\|\|flow\.screen==='request'/);assert.match(root,/flow\.screen==='request'&&<ClientNeedScreen/);assert.match(root,/!canonical&&!detailOpen&&<ClientHugoBridge/)})
test('Hugo companion is wired to the canonical voice dock, real client actions and intents',()=>{assert.match(bridge,/ClientVoiceHugoDock/);assert.match(bridge,/clientActions=\{flow\.actions\}/);assert.match(bridge,/onIntent=\{flow\.publishHugoIntent\}/);assert.match(bridge,/services=\{services\}/)})

...
test('home/request keep transactional checkout hidden while the Hugo orb stays available',()=>{assert.match(root,/canonical=flow\.screen==='home'\|\|flow\.screen==='request'/);assert.match(root,/flow\.screen==='request'&&<ClientNeedScreen/);assert.match(globals,/<ClientHugoBridge\/>/);assert.match(surfaces,/legacyOperational=!canonical&&\(screen==='service'\|\|screen==='payment'\|\|screen==='review'\|\|screen==='matching'\)/);assert.match(surfaces,/legacyOperational&&!detailOpen&&<ClientPaymentChoice/);assert.match(voice,/ugo-hugo-voice-controller/);assert.doesNotMatch(voice,/ugo-hugo-stage-card/)})

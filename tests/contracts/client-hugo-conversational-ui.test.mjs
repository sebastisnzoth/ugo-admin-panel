...
test('voice controller keeps multi-order context and service-scoped cancellation logic',()=>{assert.match(voice,/ugo-hugo-voice-controller/);assert.match(voice,/service\.categoria\?\.nombre/);assert.match(voice,/service\?\.proveedor\?\.nombre/);assert.match(voice,/services\.length>1/);assert.match(voice,/pedidos independientes/);assert.match(voice,/resolveServiceCandidates/);assert.match(voice,/cancelService\(pending\.serviceId\)/);assert.doesNotMatch(voice,/ugo-hugo-stage-card/)})

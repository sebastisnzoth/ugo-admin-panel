...
test('critical realtime consumers recover from missed events using persisted state',async()=>{
 ...
 assert.match(notifications,/status==='CHANNEL_ERROR'\|\|status==='TIMED_OUT'\|\|status==='CLOSED'/)
 assert.match(provider,/status==='CHANNEL_ERROR'\|\|status==='TIMED_OUT'\|\|status==='CLOSED'/)

...
declare
  ...
  v_ref text;
  v_cliente_demo boolean := false;
  v_proveedor_demo boolean := false;
begin
  ...
  select coalesce(es_demo,false) into v_cliente_demo from public.usuarios where id=v_servicio.cliente_id;
  select coalesce(es_demo,false) into v_proveedor_demo from public.usuarios where id=v_servicio.proveedor_id;
  if not v_cliente_demo or not v_proveedor_demo then
    raise exception 'Pago DEMO disponible solo cuando cliente y proveedor son cuentas demo';
  end if;
  ...
  values(
    ...
    'pago_retenido',
    'Pago DEMO confirmado',
    'Pago ficticio confirmado para probar el flujo del servicio #' || coalesce(v_servicio.numero::text, left(v_servicio.id::text,8)),

...
begin
  ...
  if not found then
    raise exception 'Ampliación no encontrada';
  end if;

  if v_row.ajuste_estado = 'reembolsado' and v_row.pago_estado = 'pendiente_ajuste' then
    return v_row;
  end if;
  ...
  if p_pago_externo_id is null or btrim(p_pago_externo_id) = '' then
    raise exception 'Falta referencia externa del reembolso';
  end if;
  ...
  if abs(coalesce(p_monto, 0) - v_monto) >= 0.01 then
    raise exception 'El monto reembolsado no coincide con la ampliación';
  end if;
  ...
  update public.ampliaciones_servicio
     set pago_estado = 'pendiente_ajuste',
         ajuste_estado = 'reembolsado',
  ...
  values(
    v_row.proveedor_id,
    'ampliacion_reembolsada',
    'Trabajo adicional reembolsado',
    v_row.descripcion,
    jsonb_build_object(
      'servicio_id', v_row.servicio_id,
      'ampliacion_id', v_row.id,
      'ajuste_estado', v_row.ajuste_estado,

-- Make automatic matching retry idempotent while live offers still exist.
-- A user pressing "Reintentar búsqueda" must not destroy pending offers that
-- providers can still accept. Only when no live pending offers remain do we
-- expire stale rows and search for new candidates.

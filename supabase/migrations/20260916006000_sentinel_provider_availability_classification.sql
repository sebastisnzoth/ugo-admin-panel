-- UGO TEST · Extend Sentinel server-side classification for provider availability failures.
-- Only confirmed provider.availability failures map to MATCH-ONLINE.
-- provider.availability.recovery stays unclassified because persistence was not verified.
...
begin
  ...
  if p_severity not in ('P0','P1','P2','P3') then raise exception 'INVALID_SEVERITY'; end if;
  if p_source_role not in ('client','provider','admin','system','unknown') then raise exception 'INVALID_SOURCE_ROLE'; end if;
  if nullif(trim(p_event_type),'') is null or nullif(trim(p_message),'') is null then raise exception 'INCIDENT_DETAILS_REQUIRED'; end if;
  ...
  v_checklist_code := case
    ...
    when p_action in ('client.request.matching','client.provider_radar.sync','provider.availability') then 'MATCH-ONLINE'
    ...
    when p_action = 'provider.agenda.load' then 'PROVIDER-AGENDA'

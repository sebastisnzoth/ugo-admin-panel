-- P0 Cliente retry -> Proveedor alert convergence.
-- Reintentar una búsqueda con ofertas todavía vivas debe abrir un ciclo nuevo
-- de alerta sin duplicar la oferta ni cambiar de serviceId.
...
begin
 ...
 where servicio_id=p_servicio_id and estado='pendiente' and (expira_at is null or expira_at>now());

 if v_live_count>0 then
  -- This is an explicit client retry. Refresh the existing offer cycle so
  -- trg_notify_provider_new_offer emits a fresh nueva_oferta notification.
  -- The unique (servicio_id,proveedor_id) offer row is preserved.
 ...
 update public.servicios
    set estado=case when v_count>0 then 'ofrecido'::public.servicio_estado else 'buscando'::public.servicio_estado end,updated_at=now()

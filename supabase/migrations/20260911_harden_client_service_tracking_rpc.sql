-- UGO · Hardening del tracking exacto de servicio
-- Usa comparaciones NULL-safe para que ningún tercero pueda atravesar el guard
-- de autorización aunque un servicio aún no tenga proveedor asignado.
...
begin
  ...
  if v_servicio.cliente_id is distinct from auth.uid()
     and v_servicio.proveedor_id is distinct from auth.uid()

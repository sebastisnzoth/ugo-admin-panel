-- UGO TEST · Server-side Sentinel action classification.
-- Runtime reporters may propose a checklist code, but non-admin classifications
-- are always derived here from a known instrumented action. Runtime incidents
-- remain isolated from checklist mutation and are scoped to the emitting build.
...
begin
  ...
  -- Runtime telemetry never mutates development_checklist. The dashboard combines
  -- Admin-owned readiness with current-build incidents as two independent signals.
  return v_id;
end;
$$;
